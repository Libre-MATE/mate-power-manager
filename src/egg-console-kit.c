...
enum { EGG_CONSOLE_KIT_ACTIVE_CHANGED, EGG_CONSOLE_KIT_LAST_SIGNAL };
...
gboolean egg_console_kit_restart(EggConsoleKit *console, GError **error) {
  ...
  if (!ret) {
    ...
    if (error != NULL) *error = g_error_new(1, 0, "%s", error_local->message);
 ...
 **/
gboolean egg_console_kit_hibernate(EggConsoleKit *console, GError **error) {
  gboolean ret;
  GError *error_local = NULL;
  ...
  ret =
      dbus_g_proxy_call(console->priv->proxy_manager, "Hibernate", &error_local,
                        G_TYPE_BOOLEAN, TRUE, G_TYPE_INVALID, G_TYPE_INVALID);
  if (!ret) {
 ...
 **/
gboolean egg_console_kit_can_stop(EggConsoleKit *console, gboolean *can_stop,
                                  GError **error) {
  gboolean ret;
  GError *error_local = NULL;
  ...
  ret = dbus_g_proxy_call(console->priv->proxy_manager, "CanStop", &error_local,
                          G_TYPE_INVALID, G_TYPE_BOOLEAN, can_stop,
                          G_TYPE_INVALID);
  if (!ret) {
...
gboolean egg_console_kit_can_restart(EggConsoleKit *console,
  ...
  ret = dbus_g_proxy_call(console->priv->proxy_manager, "CanRestart",
                          &error_local, G_TYPE_INVALID, G_TYPE_BOOLEAN,
                          can_restart, G_TYPE_INVALID);
  if (!ret) {
...
gboolean egg_console_kit_can_suspend(EggConsoleKit *console,
  ...
  ret = dbus_g_proxy_call(console->priv->proxy_manager, "CanSuspend",
                          &error_local, G_TYPE_INVALID, G_TYPE_STRING, &retval,
                          G_TYPE_INVALID);
  if (!ret) {
  ...
  *can_suspend =
      g_strcmp0(retval, "yes") == 0 || g_strcmp0(retval, "challenge") == 0;
...
gboolean egg_console_kit_can_hibernate(EggConsoleKit *console,
  ...
  ret = dbus_g_proxy_call(console->priv->proxy_manager, "CanHibernate",
                          &error_local, G_TYPE_INVALID, G_TYPE_STRING, &retval,
                          G_TYPE_INVALID);
  if (!ret) {
...
gboolean egg_console_kit_is_local(EggConsoleKit *console) {
  ...
  ret =
      dbus_g_proxy_call(console->priv->proxy_session, "IsLocal", &error,
                        G_TYPE_INVALID, G_TYPE_BOOLEAN, &value, G_TYPE_INVALID);
  if (!ret) {
...
gboolean egg_console_kit_is_active(EggConsoleKit *console) {
  ...
  ret =
      dbus_g_proxy_call(console->priv->proxy_session, "IsActive", &error,
                        G_TYPE_INVALID, G_TYPE_BOOLEAN, &value, G_TYPE_INVALID);
  if (!ret) {
 ...
 **/
static void egg_console_kit_active_changed_cb(DBusGProxy *proxy,
                                              gboolean active,
...
static void egg_console_kit_class_init(EggConsoleKitClass *klass) {
  ...
  signals[EGG_CONSOLE_KIT_ACTIVE_CHANGED] = g_signal_new(
      "active-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EggConsoleKitClass, active_changed), NULL, NULL,
...
static void egg_console_kit_init(EggConsoleKit *console) {
  ...
  console->priv->proxy_manager = dbus_g_proxy_new_for_name(
      console->priv->connection, CONSOLEKIT_NAME, CONSOLEKIT_MANAGER_PATH,
  ...
  ret = dbus_g_proxy_call(console->priv->proxy_manager,
                          "GetSessionForUnixProcess", &error, G_TYPE_UINT, pid,
                          G_TYPE_INVALID, DBUS_TYPE_G_OBJECT_PATH,
                          &console->priv->session_id, G_TYPE_INVALID);
  if (!ret) {
  ...
  /* connect to session */
  console->priv->proxy_session = dbus_g_proxy_new_for_name(
      console->priv->connection, CONSOLEKIT_NAME, console->priv->session_id,
