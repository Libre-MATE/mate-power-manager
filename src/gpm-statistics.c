...
enum { GPM_INFO_COLUMN_TEXT, GPM_INFO_COLUMN_VALUE, GPM_INFO_COLUMN_LAST };
 ...
 **/
static void gpm_stats_button_help_cb(GtkWidget *widget, gboolean data) {
 ...
 **/
static void gpm_stats_add_info_columns(GtkTreeView *treeview) {
  ...
  column = gtk_tree_view_column_new_with_attributes(
      _("Attribute"), renderer, "markup", GPM_INFO_COLUMN_TEXT, NULL);
 ...
 **/
static void gpm_stats_add_devices_columns(GtkTreeView *treeview) {
  GtkCellRenderer *renderer;
  GtkTreeViewColumn *column;
  ...
  column = gtk_tree_view_column_new_with_attributes(
      _("Image"), renderer, "icon-name", GPM_DEVICES_COLUMN_ICON, NULL);
  gtk_tree_view_append_column(treeview, column);

  /* column for text */
  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Description"), renderer, "markup", GPM_DEVICES_COLUMN_TEXT, NULL);
  gtk_tree_view_column_set_sort_column_id(column, GPM_INFO_COLUMN_TEXT);
  gtk_tree_view_append_column(treeview, column);
 ...
 **/
static void gpm_stats_add_info_data(const gchar *attr, const gchar *text) {
  GtkTreeIter iter;
  gtk_list_store_append(list_store_info, &iter);
  gtk_list_store_set(list_store_info, &iter, GPM_INFO_COLUMN_TEXT, attr,
 ...
 **/
static GPtrArray *gpm_stats_update_smooth_data(GPtrArray *list) {
  guint i;
  GpmPointObj *point;
 ...
 **/
static gchar *gpm_stats_time_to_string(gint seconds) {
  ...
  if (value < 60) {
    ...
    return g_strdup_printf(ngettext("%.0f second", "%.0f seconds", value),
                           value);
  }
  value /= 60.0;
  if (value < 60) {
    /* TRANSLATORS: this is a time value, usually to show on a graph */
    return g_strdup_printf(ngettext("%.1f minute", "%.1f minutes", value),
                           value);
  }
  value /= 60.0;
  if (value < 60) {
 ...
 **/
static const gchar *gpm_stats_bool_to_string(gboolean ret) {
 ...
 **/
static gchar *gpm_stats_get_printable_device_path(UpDevice *device) {
 ...
 **/
static void gpm_stats_update_info_page_details(UpDevice *device) {
  struct tm *time_tm;
  time_t t;
  gchar time_buf[256];
  gchar *text;
  ...
  g_object_get(device, "kind", &kind, "state", &state, "percentage",
               &percentage, "online", &online, "update_time", &update_time,
               "power_supply", &power_supply, "is_rechargeable",
               &is_rechargeable, "is-present", &is_present, "time-to-full",
               &time_to_full, "time-to-empty", &time_to_empty, "technology",
               &technology, "capacity", &capacity, "energy", &energy,
               "energy-empty", &energy_empty, "energy-full", &energy_full,
               "energy-full-design", &energy_full_design, "energy-rate",
               &energy_rate, "voltage", &voltage, "vendor", &vendor, "serial",
               &serial, "model", &model, NULL);
  ...
  text = g_strdup_printf(ngettext("%d second", "%d seconds", refreshed),
                         refreshed);
  ...
  if (kind == UP_DEVICE_KIND_BATTERY || kind == UP_DEVICE_KIND_MOUSE ||
      kind == UP_DEVICE_KIND_KEYBOARD || kind == UP_DEVICE_KIND_UPS) {
  ...
  if (kind == UP_DEVICE_KIND_BATTERY || kind == UP_DEVICE_KIND_MOUSE ||
    ...
    gpm_stats_add_info_data(_("Rechargeable"),
                            gpm_stats_bool_to_string(is_rechargeable));
  }
  if (kind == UP_DEVICE_KIND_BATTERY || kind == UP_DEVICE_KIND_MOUSE ||
      kind == UP_DEVICE_KIND_KEYBOARD) {
    /* TRANSLATORS: The state of the device, e.g. "Changing" or "Fully charged"
     */
    gpm_stats_add_info_data(_("State"),
                            gpm_device_state_to_localised_string(state));
  }
  ...
  if (kind == UP_DEVICE_KIND_BATTERY || kind == UP_DEVICE_KIND_MONITOR) {
  ...
  if (kind == UP_DEVICE_KIND_BATTERY) {
     ...
     */
    gpm_stats_add_info_data(
        _("Technology"), gpm_device_technology_to_localised_string(technology));
  }
 ...
 **/
static void gpm_stats_set_graph_data(GtkWidget *widget, GPtrArray *data,
                                     gboolean use_smoothed,
                                     gboolean use_points) {
  GPtrArray *smoothed;
 ...
 **/
static void gpm_stats_update_info_page_history(UpDevice *device) {
  GPtrArray *array;
  guint i;
  UpHistoryItem *item;
  GtkWidget *widget;
  ...
  if (history_type == GPM_HISTORY_CHARGE_TYPE) {
    g_object_set(graph_history, "type-x", GPM_GRAPH_WIDGET_TYPE_TIME, "type-y",
                 GPM_GRAPH_WIDGET_TYPE_PERCENTAGE, "autorange-x", FALSE,
                 "start-x", -history_time, "stop-x", 0, "autorange-y", FALSE,
                 "start-y", 0, "stop-y", 100, NULL);
  } else if (history_type == GPM_HISTORY_RATE_TYPE) {
    g_object_set(graph_history, "type-x", GPM_GRAPH_WIDGET_TYPE_TIME, "type-y",
                 GPM_GRAPH_WIDGET_TYPE_POWER, "autorange-x", FALSE, "start-x",
                 -history_time, "stop-x", 0, "autorange-y", TRUE, NULL);
  } else {
    g_object_set(graph_history, "type-x", GPM_GRAPH_WIDGET_TYPE_TIME, "type-y",
                 GPM_GRAPH_WIDGET_TYPE_TIME, "autorange-x", FALSE, "start-x",
                 -history_time, "stop-x", 0, "autorange-y", TRUE, NULL);
  }

  widget = GTK_WIDGET(gtk_builder_get_object(builder, "label_history_nodata"));
  /* The type of history, history_types [history_type], known values are "rate"
   * and "charge". */
  array = up_device_get_history_sync(device, history_types[history_type],
                                     history_time, 150, NULL, NULL);
  if (array == NULL) {
  ...
  for (i = 0; i < array->len; i++) {
    ...
    /* abandon this point */
    if (up_history_item_get_state(item) == UP_DEVICE_STATE_UNKNOWN) continue;
    ...
    else if (up_history_item_get_state(item) ==
  ...
  sigma_smoothing = 2.0;
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "checkbutton_smooth_history"));
  checked = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "checkbutton_points_history"));
 ...
 **/
static void gpm_stats_update_info_page_stats(UpDevice *device) {
  GPtrArray *array;
  guint i;
  UpStatsItem *item;
  GtkWidget *widget;
  ...
  if (use_data) {
    g_object_set(graph_statistics, "type-x", GPM_GRAPH_WIDGET_TYPE_PERCENTAGE,
                 "type-y", GPM_GRAPH_WIDGET_TYPE_FACTOR, "autorange-x", TRUE,
                 "autorange-y", TRUE, NULL);
  } else {
    g_object_set(graph_statistics, "type-x", GPM_GRAPH_WIDGET_TYPE_PERCENTAGE,
                 "type-y", GPM_GRAPH_WIDGET_TYPE_PERCENTAGE, "autorange-x",
                 TRUE, "autorange-y", TRUE, NULL);
  }
  ...
  sigma_smoothing = 1.1;
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "checkbutton_smooth_stats"));
  checked = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "checkbutton_points_stats"));
  points = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
 ...
 **/
static void gpm_stats_update_info_data_page(UpDevice *device, gint page) {
  if (page == 0)
 ...
 **/
static void gpm_stats_update_info_data(UpDevice *device) {
  gint page;
  ...
  /* get properties */
  g_object_get(device, "has-history", &has_history, "has-statistics",
               &has_statistics, NULL);
...
static void gpm_stats_set_title(GtkWindow *window, gint page_num) {
  gchar *title;
  ...
  /* TRANSLATORS: shown on the titlebar */
  title = g_strdup_printf("%s - %s", _("Power Statistics"),
                          _(page_titles[page_num]));
 ...
 **/
static void gpm_stats_notebook_changed_cb(GtkNotebook *notebook, gpointer page,
 ...
 **/
static void gpm_stats_button_update_ui(void) {
  UpDevice *device;
 ...
 **/
static void gpm_stats_devices_treeview_clicked_cb(GtkTreeSelection *selection,
                                                  gboolean data) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  UpDevice *device;
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, GPM_DEVICES_COLUMN_ID, &current_device,
                       -1);

    /* save device in gsettings */
    g_settings_set_string(settings, GPM_SETTINGS_INFO_LAST_DEVICE,
                          current_device);
 ...
 **/
static void gpm_stats_window_activated_cb(GtkApplication *app, gpointer data) {
  GtkWidget *widget;
  widget = GTK_WIDGET(gtk_builder_get_object(builder, "dialog_stats"));
 ...
 **/
static void gpm_stats_device_changed_cb(UpDevice *device, GParamSpec *pspec,
                                        gpointer user_data) {
  const gchar *object_path;
  object_path = up_device_get_object_path(device);
  if (object_path == NULL || current_device == NULL) return;
 ...
 **/
static void gpm_stats_add_device(UpDevice *device, GPtrArray *devices) {
  const gchar *id;
  GtkTreeIter iter;
  char *icon;
  UpDeviceKind kind;
  ...
  if (devices != NULL) g_ptr_array_add(devices, device);

  g_signal_connect(device, "notify", G_CALLBACK(gpm_stats_device_changed_cb),
                   NULL);

  /* get device properties */
  g_object_get(device, "kind", &kind, "vendor", &vendor, "model", &model, NULL);

  id = up_device_get_object_path(device);
  if ((vendor != NULL && strlen(vendor) != 0) &&
      (model != NULL && strlen(model) != 0)) {
    label = g_strdup_printf("%s %s", vendor, model);
  } else if ((vendor == NULL || strlen(vendor) == 0) &&
             (model != NULL && strlen(model) != 0)) {
  ...
  gtk_list_store_set(list_store_devices, &iter, GPM_DEVICES_COLUMN_ID, id,
                     GPM_DEVICES_COLUMN_TEXT, label, GPM_DEVICES_COLUMN_ICON,
                     icon, -1);
  g_free(icon);
  g_free(label);
  g_free(vendor);
 ...
 **/
static void gpm_stats_device_added_cb(UpClient *client, UpDevice *device,
                                      GPtrArray *devices) {
  const gchar *object_path;
  object_path = up_device_get_object_path(device);
 ...
 **/
static void gpm_stats_device_removed_cb(UpClient *client,
                                        const gchar *object_path,
                                        GPtrArray *devices) {
  GtkTreeIter iter;
  gchar *id = NULL;
  ...
  ret =
      gtk_tree_model_get_iter_first(GTK_TREE_MODEL(list_store_devices), &iter);
  while (ret) {
    gtk_tree_model_get(GTK_TREE_MODEL(list_store_devices), &iter,
                       GPM_DEVICES_COLUMN_ID, &id, -1);
 ...
 **/
static void gpm_stats_history_type_combo_changed_cb(GtkWidget *widget,
                                                    gpointer data) {
  guint active;
  ...
  /* save to gsettings */
  g_settings_set_string(settings, GPM_SETTINGS_INFO_HISTORY_TYPE,
 ...
 **/
static void gpm_stats_type_combo_changed_cb(GtkWidget *widget, gpointer data) {
  guint active;
  const gchar *axis_x = NULL;
  const gchar *axis_y = NULL;
  ...
  /* save to gsettings */
  g_settings_set_string(settings, GPM_SETTINGS_INFO_STATS_TYPE,
 ...
 **/
static void gpm_stats_range_combo_changed(GtkWidget *widget, gpointer data) {
  guint active;
 ...
 **/
static void gpm_stats_smooth_checkbox_history_cb(GtkWidget *widget,
                                                 gpointer data) {
  gboolean checked;
  checked = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  g_settings_set_boolean(settings, GPM_SETTINGS_INFO_HISTORY_GRAPH_SMOOTH,
                         checked);
  gpm_stats_button_update_ui();
}
 ...
 * @widget: The GtkWidget object
 **/
static void gpm_stats_smooth_checkbox_stats_cb(GtkWidget *widget,
                                               gpointer data) {
  gboolean checked;
  checked = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  g_settings_set_boolean(settings, GPM_SETTINGS_INFO_STATS_GRAPH_SMOOTH,
                         checked);
  gpm_stats_button_update_ui();
}
 ...
 * @widget: The GtkWidget object
 **/
static void gpm_stats_points_checkbox_history_cb(GtkWidget *widget,
                                                 gpointer data) {
  gboolean checked;
  checked = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  g_settings_set_boolean(settings, GPM_SETTINGS_INFO_HISTORY_GRAPH_POINTS,
                         checked);
  gpm_stats_button_update_ui();
}
 ...
 * @widget: The GtkWidget object
 **/
static void gpm_stats_points_checkbox_stats_cb(GtkWidget *widget,
                                               gpointer data) {
  gboolean checked;
  checked = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  g_settings_set_boolean(settings, GPM_SETTINGS_INFO_STATS_GRAPH_POINTS,
                         checked);
  gpm_stats_button_update_ui();
}
 ...
 **/
static void gpm_stats_highlight_device(const gchar *object_path) {
  gboolean ret;
  gchar *id = NULL;
  gchar *path_str;
  guint i;
  ...
  /* check valid */
  if (!g_str_has_prefix(object_path, "/")) return;
  ...
  for (i = 0; ret; i++) {
    ...
    if (g_strcmp0(id, object_path) == 0) {
      ...
      gtk_tree_view_set_cursor_on_cell(GTK_TREE_VIEW(widget), path, NULL, NULL,
                                       FALSE);
      g_free(path_str);
...
int main(int argc, char *argv[]) {
  ...
  g_signal_connect(app, "activate", G_CALLBACK(gpm_stats_window_activated_cb),
                   NULL);
  ...
  builder = gtk_builder_new_from_resource(
      "/org/mate/powermanager/statistics/gpm-statistics.ui");
  ...
  g_signal_connect_swapped(widget, "clicked", G_CALLBACK(gtk_widget_destroy),
                           window);
  gtk_widget_grab_default(widget);

  widget = GTK_WIDGET(gtk_builder_get_object(builder, "button_help"));
  g_signal_connect(widget, "clicked", G_CALLBACK(gpm_stats_button_help_cb),
                   NULL);

  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "checkbutton_smooth_history"));
  checked =
      g_settings_get_boolean(settings, GPM_SETTINGS_INFO_HISTORY_GRAPH_SMOOTH);
  ...
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "checkbutton_smooth_stats"));
  checked =
      g_settings_get_boolean(settings, GPM_SETTINGS_INFO_STATS_GRAPH_SMOOTH);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), checked);
  g_signal_connect(widget, "clicked",
  ...
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "checkbutton_points_history"));
  checked =
      g_settings_get_boolean(settings, GPM_SETTINGS_INFO_HISTORY_GRAPH_POINTS);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), checked);
  g_signal_connect(widget, "clicked",
  ...
  widget =
      GTK_WIDGET(gtk_builder_get_object(builder, "checkbutton_points_stats"));
  checked =
      g_settings_get_boolean(settings, GPM_SETTINGS_INFO_STATS_GRAPH_POINTS);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), checked);
  g_signal_connect(widget, "clicked",
  ...
  /* create list stores */
  list_store_info =
      gtk_list_store_new(GPM_INFO_COLUMN_LAST, G_TYPE_STRING, G_TYPE_STRING);
  list_store_devices = gtk_list_store_new(
      GPM_DEVICES_COLUMN_LAST, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
  ...
  if ((history_type_str == NULL) ||
      (strcmp(history_type_str, GPM_HISTORY_CHARGE_VALUE) == 0)) {
    history_type = GPM_HISTORY_CHARGE_TYPE;
  ...
  if (history_time == 0) history_time = GPM_HISTORY_HOUR_VALUE;

  char *stats_type_str =
      g_settings_get_string(settings, GPM_SETTINGS_INFO_STATS_TYPE);
  if ((stats_type_str == NULL) ||
      (strcmp(stats_type_str, GPM_STATS_CHARGE_DATA_VALUE) == 0)) {
    stats_type = GPM_STATS_CHARGE_TYPE;
  ...
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(widget),
                                 GPM_HISTORY_MINUTE_TEXT);
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(widget),
                                 GPM_HISTORY_HOUR_TEXT);
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(widget),
  ...
  for (j = 0; j < UP_DEVICE_KIND_LAST; j++) {
    for (i = 0; i < devices->len; i++) {
      ...
      if (kind == j) /* NULL == do not add it to ptr array */
  ...
  g_signal_connect(client, "device-added",
                   G_CALLBACK(gpm_stats_device_added_cb), devices);
  g_signal_connect(client, "device-removed",
                   G_CALLBACK(gpm_stats_device_removed_cb), devices);
  ...
  if (last_device == NULL)
    last_device =
        g_settings_get_string(settings, GPM_SETTINGS_INFO_LAST_DEVICE);

  /* set the correct focus on the last device */
  if (last_device != NULL) gpm_stats_highlight_device(last_device);
  ...
  if (devices != NULL) g_ptr_array_unref(devices);
