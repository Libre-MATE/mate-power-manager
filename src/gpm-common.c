...
gchar *gpm_get_timestring(guint time_secs) {
  ...
  if (minutes < 60) {
    timestring =
        g_strdup_printf(ngettext("%i minute", "%i minutes", minutes), minutes);
    return timestring;
  }
  ...
  if (minutes == 0)
    ...
    timestring = g_strdup_printf(_("%i %s %i %s"), hours,
                                 ngettext("hour", "hours", hours), minutes,
...
guint gpm_discrete_from_percent(guint percentage, guint levels) {
  ...
  if (percentage > 100) return levels;
  if (levels == 0) {
    g_warning("levels is 0!");
    return 0;
  }
  return (guint)((((gfloat)percentage * (gfloat)(levels - factor)) / 100.0f) +
                 0.5f);
}
...
guint gpm_discrete_to_percent(guint discrete, guint levels) {
  ...
  factor = levels < 10 ? 0 : 1;
  /* check we are in range */
  if (discrete > levels) return 100;
  if (levels == 0) {
    g_warning("levels is 0!");
    return 0;
  }
  return (guint)(((gfloat)discrete * (100.0f / (gfloat)(levels - factor))) +
                 0.5f);
}
...
void gpm_common_test(gpointer data) {
  ...
  if (egg_test_start(test, "GpmCommon") == FALSE) return;
