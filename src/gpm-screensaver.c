...
gboolean gpm_screensaver_lock(GpmScreensaver *screensaver) {
  ...
  dbus_g_proxy_call_no_reply(screensaver->priv->proxy, "Lock", G_TYPE_INVALID);
 ...
 **/
guint gpm_screensaver_add_throttle(GpmScreensaver *screensaver,
                                   const char *reason) {
  GError *error = NULL;
  gboolean ret;
  guint32 cookie;
  ...
  ret = dbus_g_proxy_call(screensaver->priv->proxy, "Throttle", &error,
                          G_TYPE_STRING, "Power screensaver", G_TYPE_STRING,
                          reason, G_TYPE_INVALID, G_TYPE_UINT, &cookie,
                          G_TYPE_INVALID);
  if (error) {
...
gboolean gpm_screensaver_remove_throttle(GpmScreensaver *screensaver,
  ...
  ret = dbus_g_proxy_call(screensaver->priv->proxy, "UnThrottle", &error,
...
gboolean gpm_screensaver_check_running(GpmScreensaver *screensaver) {
  ...
  ret =
      dbus_g_proxy_call(screensaver->priv->proxy, "GetActive", &error,
                        G_TYPE_INVALID, G_TYPE_BOOLEAN, &temp, G_TYPE_INVALID);
  if (error) {
    g_debug("ERROR: %s", error->message);
    g_error_free(error);
  }
...
gboolean gpm_screensaver_poke(GpmScreensaver *screensaver) {
  ...
  g_debug("poke");
  dbus_g_proxy_call_no_reply(screensaver->priv->proxy, "SimulateUserActivity",
                             G_TYPE_INVALID);
  return TRUE;
}
...
static void gpm_screensaver_init(GpmScreensaver *screensaver) {
  ...
  screensaver->priv->proxy = dbus_g_proxy_new_for_name(
      connection, GS_LISTENER_SERVICE, GS_LISTENER_PATH, GS_LISTENER_INTERFACE);
}
...
void gpm_screensaver_test(gpointer data) {
  ...
  if (egg_test_start(test, "GpmScreensaver") == FALSE) return;
