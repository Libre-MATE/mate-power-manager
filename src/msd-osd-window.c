...
enum { DRAW_WHEN_COMPOSITED, LAST_SIGNAL };
...
static gboolean hide_timeout(MsdOsdWindow *window) {
  if (window->priv->is_composited) {
    ...
    window->priv->fade_timeout_id =
        g_timeout_add(FADE_TIMEOUT, (GSourceFunc)fade_timeout, window);
  } else {
    gtk_widget_hide(GTK_WIDGET(window));
  }
...
static void add_hide_timeout(MsdOsdWindow *window) {
  ...
  window->priv->hide_timeout_id =
      g_timeout_add(timeout, (GSourceFunc)hide_timeout, window);
}
...
static void draw_when_composited(GtkWidget *widget, cairo_t *orig_cr) {
  ...
  surface = cairo_surface_create_similar(
      cairo_get_target(orig_cr), CAIRO_CONTENT_COLOR_ALPHA, width, height);
...
/* This is our expose/draw-event handler when the window is *not* in a
 * compositing manager. We just draw a rectangular frame by hand.  We do this
 * with hardcoded drawing code, instead of GtkFrame, to avoid changing the
 * window's internal widget hierarchy:  in either case (composited or
 * non-composited), callers can assume that this works identically to a
 * GtkWindow without any intermediate widgetry.
 */
...
static gboolean msd_osd_window_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (child) gtk_container_propagate_draw(GTK_CONTAINER(window), child, cr);
...
static void msd_osd_window_get_preferred_width(GtkWidget *widget, gint *minimum,
                                               gint *natural) {
  GtkStyleContext *context;
  GtkBorder padding;
  ...
  /* See the comment in msd_osd_window_style_updated() for why we add the
   * padding here */
...
static void msd_osd_window_get_preferred_height(GtkWidget *widget,
...
static GObject *msd_osd_window_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_params) {
  GObject *object;

  object = G_OBJECT_CLASS(msd_osd_window_parent_class)
               ->constructor(type, n_construct_properties, construct_params);

  g_object_set(object, "type", GTK_WINDOW_POPUP, "type-hint",
               GDK_WINDOW_TYPE_HINT_NOTIFICATION, "skip-taskbar-hint", TRUE,
...
static void msd_osd_window_class_init(MsdOsdWindowClass *klass) {
  ...
  signals[DRAW_WHEN_COMPOSITED] = g_signal_new(
      "draw-when-composited", G_TYPE_FROM_CLASS(gobject_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MsdOsdWindowClass, draw_when_composited), NULL, NULL,
 ...
 * @window: a #MsdOsdWindow
 *
 * Queues the @window for immediate drawing, and queues a timer to hide the
 * window.
 */
