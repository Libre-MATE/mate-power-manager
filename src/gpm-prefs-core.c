...
#define GET_WIDGET(x) \
  (GTK_WIDGET(gtk_builder_get_object(prefs->priv->builder, (x))))
#define GET_NOTEBOOK(x) \
  (GTK_NOTEBOOK(gtk_builder_get_object(prefs->priv->builder, (x))))
#define GET_WINDOW(x) \
  (GTK_WINDOW(gtk_builder_get_object(prefs->priv->builder, (x))))
...
static void gpm_prefs_class_init(GpmPrefsClass *klass) {
  ...
  signals[ACTION_HELP] = g_signal_new(
      "action-help", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmPrefsClass, action_help), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[ACTION_CLOSE] = g_signal_new(
      "action-close", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmPrefsClass, action_close), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
 ...
 * @value: The value in %.
 **/
static gchar *gpm_prefs_format_percentage_cb(GtkScale *scale, gdouble value) {
 ...
 **/
static void gpm_prefs_action_combo_changed_cb(GtkWidget *widget,
                                              GpmPrefs *prefs) {
  ...
  actions =
      (const GpmActionPolicy *)g_object_get_data(G_OBJECT(widget), "actions");
  gpm_pref_key =
      (const gchar *)g_object_get_data(G_OBJECT(widget), "settings_key");
 ...
 **/
static void gpm_prefs_action_time_changed_cb(GtkWidget *widget,
                                             GpmPrefs *prefs) {
  guint value;
  const gint *values;
  const gchar *gpm_pref_key;
 ...
 **/
static void gpm_prefs_setup_action_combo(GpmPrefs *prefs,
  ...
  for (i = 0; actions_added[i] != -1; i++) {
    ...
    if (value == policy) gtk_combo_box_set_active(GTK_COMBO_BOX(widget), i);
  }
 ...
 * @actions: The actions to associate in an array.
 **/
static void gpm_prefs_setup_time_combo(GpmPrefs *prefs,
                                       const gchar *widget_name,
                                       const gchar *gpm_pref_key,
                                       const gint *values) {
  guint value;
  ...
  for (i = 0; values[i] != -1; i++) {
    ...
    if (value == values[i]) gtk_combo_box_set_active(GTK_COMBO_BOX(widget), i);
  }
 ...
 * @prefs: This prefs class instance
 **/
static gboolean gpm_prefs_delete_event_cb(GtkWidget *widget, GdkEvent *event,
                                          GpmPrefs *prefs) {
...
static void prefs_setup_ac(GpmPrefs *prefs) {
  ...
  const GpmActionPolicy button_lid_actions[] = {
      GPM_ACTION_POLICY_NOTHING,  GPM_ACTION_POLICY_BLANK,
      GPM_ACTION_POLICY_SUSPEND,  GPM_ACTION_POLICY_HIBERNATE,
      GPM_ACTION_POLICY_SHUTDOWN, -1};

  static const gint computer_times[] = {10 * 60,     30 * 60, 1 * 60 * 60,
                                        2 * 60 * 60, 0, /* never */
                                        -1};
  static const gint display_times[] = {1 * 60,  5 * 60,      10 * 60,
                                       30 * 60, 1 * 60 * 60, 0, /* never */
                                       -1};

  gpm_prefs_setup_time_combo(prefs, "combobox_ac_computer",
                             GPM_SETTINGS_SLEEP_COMPUTER_AC, computer_times);
  gpm_prefs_setup_time_combo(prefs, "combobox_ac_display",
                             GPM_SETTINGS_SLEEP_DISPLAY_AC, display_times);

  gpm_prefs_setup_action_combo(prefs, "combobox_ac_lid",
                               GPM_SETTINGS_BUTTON_LID_AC, button_lid_actions);
...
static void prefs_setup_battery(GpmPrefs *prefs) {
  ...
  const GpmActionPolicy battery_critical_actions[] = {
      GPM_ACTION_POLICY_NOTHING, GPM_ACTION_POLICY_SUSPEND,
      GPM_ACTION_POLICY_HIBERNATE, GPM_ACTION_POLICY_SHUTDOWN, -1};
  ...
  gpm_prefs_setup_time_combo(prefs, "combobox_battery_computer",
                             GPM_SETTINGS_SLEEP_COMPUTER_BATT, computer_times);
  gpm_prefs_setup_time_combo(prefs, "combobox_battery_display",
                             GPM_SETTINGS_SLEEP_DISPLAY_BATT, display_times);
...
static void prefs_setup_ups(GpmPrefs *prefs) {
  ...
  gpm_prefs_setup_time_combo(prefs, "combobox_ups_computer",
                             GPM_SETTINGS_SLEEP_COMPUTER_UPS, computer_times);
  gpm_prefs_setup_time_combo(prefs, "combobox_ups_display",
                             GPM_SETTINGS_SLEEP_DISPLAY_UPS, display_times);
  ...
  gpm_prefs_setup_action_combo(prefs, "combobox_ups_low",
                               GPM_SETTINGS_ACTION_LOW_UPS, ups_low_actions);
...
static void prefs_setup_general(GpmPrefs *prefs) {
  const GpmActionPolicy power_button_actions[] = {
      GPM_ACTION_POLICY_INTERACTIVE, GPM_ACTION_POLICY_SUSPEND,
      GPM_ACTION_POLICY_HIBERNATE,   GPM_ACTION_POLICY_SHUTDOWN,
      GPM_ACTION_POLICY_NOTHING,     -1};
  const GpmActionPolicy suspend_button_actions[] = {
      GPM_ACTION_POLICY_NOTHING, GPM_ACTION_POLICY_SUSPEND,
      GPM_ACTION_POLICY_HIBERNATE, -1};

  gpm_prefs_setup_action_combo(prefs, "combobox_general_power",
                               GPM_SETTINGS_BUTTON_POWER, power_button_actions);
...
static void gpm_prefs_init(GpmPrefs *prefs) {
  ...
  if (LOGIND_RUNNING()) {
    ...
    res = g_dbus_proxy_call_sync(proxy, "CanHibernate", NULL,
                                 G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    if (error == NULL && res != NULL) {
      g_variant_get(res, "(&s)", &r);
  ...
  if (LOGIND_RUNNING()) {
    ...
    res = g_dbus_proxy_call_sync(
        proxy, "Get",
        g_variant_new("(ss)", "org.freedesktop.UPower", "LidIsPresent"),
        G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    if (error == NULL && res != NULL) {
  ...
  for (i = 0; i < devices->len; i++) {
    ...
    if (kind == UP_DEVICE_KIND_BATTERY) prefs->priv->has_batteries = TRUE;
    if (kind == UP_DEVICE_KIND_UPS) prefs->priv->has_ups = TRUE;
  }
  ...
  (void)gtk_builder_add_from_resource(
      prefs->priv->builder, "/org/mate/powermanager/preferences/gpm-prefs.ui",
      &error);
  ...
  icon_policy =
      g_settings_get_enum(prefs->priv->settings, GPM_SETTINGS_ICON_POLICY);
  ...
  gtk_widget_set_sensitive(
      GET_WIDGET("box_general_notification"),
  ...
  gtk_builder_add_callback_symbols(
      prefs->priv->builder, "on_dialog_preferences_delete_event",
      G_CALLBACK(gpm_prefs_delete_event_cb), "on_button_help_clicked",
      G_CALLBACK(gpm_prefs_help_cb), "on_button_close_clicked",
      G_CALLBACK(gpm_prefs_close_cb),
      "on_radiobutton_notification_never_clicked",
      G_CALLBACK(gpm_prefs_icon_radio_cb),
      "on_radiobutton_notification_low_clicked",
      G_CALLBACK(gpm_prefs_icon_radio_cb),
      "on_radiobutton_notification_charge_clicked",
      G_CALLBACK(gpm_prefs_icon_radio_cb),
      "on_radiobutton_notification_present_clicked",
      G_CALLBACK(gpm_prefs_icon_radio_cb),
      "on_radiobutton_notification_always_clicked",
      G_CALLBACK(gpm_prefs_icon_radio_cb), NULL);
