...
static const gchar *gpm_idle_mode_to_string(GpmIdleMode mode) {
  if (mode == GPM_IDLE_MODE_NORMAL) return "normal";
  if (mode == GPM_IDLE_MODE_DIM) return "dim";
  if (mode == GPM_IDLE_MODE_BLANK) return "blank";
  if (mode == GPM_IDLE_MODE_SLEEP) return "sleep";
  return "unknown";
}
 ...
 **/
GpmIdleMode gpm_idle_get_mode(GpmIdle *idle) { return idle->priv->mode; }
...
static void gpm_idle_evaluate(GpmIdle *idle) {
  ...
  g_debug("session_idle=%i, idle_inhibited=%i, suspend_inhibited=%i, x_idle=%i",
          is_idle, is_idle_inhibited, is_suspend_inhibited, idle->priv->x_idle);
  ...
  if (idle->priv->timeout_blank_id == 0 && idle->priv->timeout_blank != 0) {
    g_debug("setting up blank callback for %is", idle->priv->timeout_blank);
    idle->priv->timeout_blank_id = g_timeout_add_seconds(
        idle->priv->timeout_blank, (GSourceFunc)gpm_idle_blank_cb, idle);
  ...
  if (is_suspend_inhibited) {
    ...
    /* only do the sleep timeout when the session is idle and we aren't
     * inhibited from sleeping */
    if (idle->priv->timeout_sleep_id == 0 && idle->priv->timeout_sleep != 0) {
      g_debug("setting up sleep callback %is", idle->priv->timeout_sleep);
      idle->priv->timeout_sleep_id = g_timeout_add_seconds(
          idle->priv->timeout_sleep, (GSourceFunc)gpm_idle_sleep_cb, idle);
...
gboolean gpm_idle_set_timeout_dim(GpmIdle *idle, guint timeout) {
  ...
  if (idle->priv->timeout_dim != timeout) {
    ...
    if (timeout > 0)
      egg_idletime_alarm_set(idle->priv->idletime, GPM_IDLE_IDLETIME_ID,
                             timeout * 1000);
    else
 ...
 **/
static void gpm_idle_session_idle_changed_cb(GpmSession *session,
                                             gboolean is_idle, GpmIdle *idle) {
 ...
 **/
static void gpm_idle_session_inhibited_changed_cb(GpmSession *session,
                                                  gboolean is_idle_inhibited,
                                                  gboolean is_suspend_inhibited,
                                                  GpmIdle *idle) {
  g_debug("Received mate session inhibited changed: idle=(%i), suspend=(%i)",
 ...
 **/
static void gpm_idle_idletime_alarm_expired_cb(EggIdletime *idletime,
                                               guint alarm_id, GpmIdle *idle) {
 ...
 **/
static void gpm_idle_idletime_reset_cb(EggIdletime *idletime, GpmIdle *idle) {
...
static void gpm_idle_class_init(GpmIdleClass *klass) {
  ...
  signals[IDLE_CHANGED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(GpmIdleClass, idle_changed), NULL, NULL,
...
static void gpm_idle_init(GpmIdle *idle) {
  ...
  g_signal_connect(idle->priv->session, "inhibited-changed",
                   G_CALLBACK(gpm_idle_session_inhibited_changed_cb), idle);
...
static void gpm_idle_test_idle_changed_cb(GpmIdle *idle, GpmIdleMode mode,
                                          EggTest *test) {
  _mode = mode;
...
void gpm_idle_test(gpointer data) {
  ...
  if (!egg_test_start(test, "GpmIdle")) return;
