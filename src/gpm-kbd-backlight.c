...
GQuark gpm_kbd_backlight_error_quark(void) {
  ...
  if (!quark) quark = g_quark_from_static_string("gpm_kbd_backlight_error");
  return quark;
}
...
static gboolean gpm_kbd_backlight_set(GpmKbdBacklight *backlight,
                                      guint percentage) {
  gint scale;
  guint goal;
  ...
  if (backlight->priv->max_brightness < 1) return FALSE;
  ...
  while (backlight->priv->brightness != goal) {
    ...
    backlight->priv->brightness_percent = gpm_discrete_to_percent(
        backlight->priv->brightness, backlight->priv->max_brightness);

    g_dbus_proxy_call(backlight->priv->upower_proxy, "SetBrightness",
...
static void gpm_kbd_backlight_dialog_init(GpmKbdBacklight *backlight) {
  ...
  if (backlight->priv->popup == NULL) {
    ...
    msd_media_keys_window_set_action_custom(
        MSD_MEDIA_KEYS_WINDOW(backlight->priv->popup), "gpm-brightness-kbd",
        TRUE);
 ...
 **/
static gboolean gpm_kbd_backlight_brightness_up(GpmKbdBacklight *backlight) {
  guint new;
 ...
 **/
static gboolean gpm_kbd_backlight_brightness_down(GpmKbdBacklight *backlight) {
  guint new;

  // we can possibly go below 0 here, so by converting to a gint we avoid
  // underflow errors.
  new = MAX((gint)backlight->priv->brightness_percent - GPM_KBD_BACKLIGHT_STEP,
            0);
  return gpm_kbd_backlight_set(backlight, new);
}
...
static void gpm_kbd_backlight_on_brightness_changed(GpmKbdBacklight *backlight,
                                                    guint value) {
 ...
 **/
static void gpm_kbd_backlight_on_dbus_signal(GDBusProxy *proxy,
                                             gchar *sender_name,
                                             gchar *signal_name,
                                             GVariant *parameters,
                                             gpointer user_data) {
...
static gboolean gpm_kbd_backlight_evaluate_power_source_and_set(
  ...
  battery_reduce = g_settings_get_boolean(
      backlight->priv->settings, GPM_SETTINGS_KBD_BACKLIGHT_BATT_REDUCE);
 ...
 **/
static void gpm_kbd_backlight_control_resume_cb(GpmControl *control,
                                                GpmControlAction action,
                                                GpmKbdBacklight *backlight) {
  gboolean ret;

  ret = gpm_kbd_backlight_evaluate_power_source_and_set(backlight);
  if (!ret) g_warning("Failed to turn kbd brightness back on after resuming");
}
 ...
 * @type: The button type, but here we only care about keyboard brightness
 *buttons
 * @backlight: This class instance
 **/
static void gpm_kbd_backlight_button_pressed_cb(GpmButton *button,
                                                const gchar *type,
                                                GpmKbdBacklight *backlight) {
  ...
  if (g_strcmp0(type, GPM_BUTTON_KBD_BRIGHT_UP) == 0) {
    ...
    if (ret) {
      ...
      msd_media_keys_window_set_volume_level(
          MSD_MEDIA_KEYS_WINDOW(backlight->priv->popup),
          backlight->priv->brightness_percent);
 ...
 **/
static void gpm_kbd_backlight_idle_changed_cb(GpmIdle *idle, GpmIdleMode mode,
                                              GpmKbdBacklight *backlight) {
  gfloat brightness;
  gfloat scale;
  guint value;
...
static void gpm_kbd_backlight_class_init(GpmKbdBacklightClass *klass) {
  ...
  signals[BRIGHTNESS_CHANGED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(GpmKbdBacklightClass, brightness_changed), NULL, NULL,
...
static void gpm_kbd_backlight_init(GpmKbdBacklight *backlight) {
  ...
  backlight->priv->upower_proxy = g_dbus_proxy_new_for_bus_sync(
      ...
      "org.freedesktop.UPower.KbdBacklight", NULL, &error);
  if (backlight->priv->upower_proxy == NULL) {
  ...
  g_signal_connect(backlight->priv->upower_proxy, "g-signal",
                   G_CALLBACK(gpm_kbd_backlight_on_dbus_signal), backlight);

  u_brightness =
      g_dbus_proxy_call_sync(backlight->priv->upower_proxy, "GetBrightness",
  ...
  error = NULL;
  u_max_brightness =
      g_dbus_proxy_call_sync(backlight->priv->upower_proxy, "GetMaxBrightness",
                             NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
