...
GQuark gpm_dpms_error_quark(void) {
  ...
  if (!quark) quark = g_quark_from_static_string("gpm_dpms_error");
  return quark;
}
 ...
 **/
static gboolean gpm_dpms_x11_get_mode(GpmDpms *dpms, GpmDpmsMode *mode,
                                      GError **error) {
  GpmDpmsMode result;
  BOOL enabled = FALSE;
  CARD16 state;
 ...
 **/
static gboolean gpm_dpms_x11_set_mode(GpmDpms *dpms, GpmDpmsMode mode,
                                      GError **error) {
 ...
 * gpm_dpms_set_mode:
 **/
gboolean gpm_dpms_set_mode(GpmDpms *dpms, GpmDpmsMode mode, GError **error) {
  gboolean ret;
 ...
 * gpm_dpms_get_mode:
 **/
gboolean gpm_dpms_get_mode(GpmDpms *dpms, GpmDpmsMode *mode, GError **error) {
  gboolean ret;
  if (mode) *mode = GPM_DPMS_MODE_UNKNOWN;
...
static void gpm_dpms_class_init(GpmDpmsClass *klass) {
  ...
  signals[MODE_CHANGED] = g_signal_new(
      "mode-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmDpmsClass, mode_changed), NULL, NULL,
...
static void gpm_dpms_init(GpmDpms *dpms) {
  ...
  dpms->priv->timer_id = g_timeout_add_seconds(
      GPM_DPMS_POLL_TIME, (GSourceFunc)gpm_dpms_poll_mode_cb, dpms);
  g_source_set_name_by_id(dpms->priv->timer_id, "[GpmDpms] poll");

  /* ensure we clear the default timeouts (Standby: 1200s, Suspend: 1800s, Off:
   * 2400s) */
...
void gpm_dpms_test(gpointer data) {
  ...
  if (!egg_test_start(test, "GpmDpms")) return;
