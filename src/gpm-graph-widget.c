...
G_DEFINE_TYPE_WITH_PRIVATE(GpmGraphWidget, gpm_graph_widget,
 ...
 **/
static gboolean gpm_graph_widget_key_data_clear(GpmGraphWidget *graph) {
  ...
  for (i = 0; i < g_slist_length(graph->priv->key_data); i++) {
    keyitem =
        (GpmGraphWidgetKeyData *)g_slist_nth_data(graph->priv->key_data, i);
    g_free(keyitem->desc);
    g_free(keyitem);
  }
...
gboolean gpm_graph_widget_key_data_add(GpmGraphWidget *graph, guint32 color,
  ...
  graph->priv->key_data =
      g_slist_append(graph->priv->key_data, (gpointer)keyitem);
  return TRUE;
}
 ...
 **/
static void up_graph_get_property(GObject *object, guint prop_id, GValue *value,
                                  GParamSpec *pspec) {
 ...
 **/
static void up_graph_set_property(GObject *object, guint prop_id,
 ...
 **/
static void gpm_graph_widget_class_init(GpmGraphWidgetClass *class) {
  ...
  g_object_class_install_property(
      object_class, PROP_USE_LEGEND,
      g_param_spec_boolean("use-legend", NULL, NULL, FALSE, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_USE_GRID,
      g_param_spec_boolean("use-grid", NULL, NULL, TRUE, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_TYPE_X,
      g_param_spec_uint("type-x", NULL, NULL, GPM_GRAPH_WIDGET_TYPE_INVALID,
                        GPM_GRAPH_WIDGET_TYPE_UNKNOWN,
                        GPM_GRAPH_WIDGET_TYPE_TIME, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_TYPE_Y,
      g_param_spec_uint("type-y", NULL, NULL, GPM_GRAPH_WIDGET_TYPE_INVALID,
                        GPM_GRAPH_WIDGET_TYPE_UNKNOWN,
                        GPM_GRAPH_WIDGET_TYPE_PERCENTAGE, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_AUTORANGE_X,
      g_param_spec_boolean("autorange-x", NULL, NULL, TRUE, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_AUTORANGE_Y,
      g_param_spec_boolean("autorange-y", NULL, NULL, TRUE, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_START_X,
      g_param_spec_int("start-x", NULL, NULL, G_MININT, G_MAXINT, 0,
                       G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_START_Y,
      g_param_spec_int("start-y", NULL, NULL, G_MININT, G_MAXINT, 0,
                       G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_STOP_X,
      g_param_spec_int("stop-x", NULL, NULL, G_MININT, G_MAXINT, 60,
                       G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_STOP_Y,
      g_param_spec_int("stop-y", NULL, NULL, G_MININT, G_MAXINT, 100,
                       G_PARAM_READWRITE));
}
 ...
 **/
static void gpm_graph_widget_init(GpmGraphWidget *graph) {
 ...
 **/
static gchar *gpm_get_axis_label(GpmGraphWidgetType axis, gfloat value) {
  gchar *text = NULL;
 ...
 **/
static void gpm_graph_widget_draw_grid(GpmGraphWidget *graph, cairo_t *cr) {
  guint i;
  gfloat b;
  gdouble dotted[] = {1., 2.};
  ...
  for (i = 1; i < 10; i++) {
    ...
    cairo_line_to(cr, (gint)b + 0.5f,
                  graph->priv->box_y + graph->priv->box_height);
    cairo_stroke(cr);
  }
  ...
  for (i = 1; i < 10; i++) {
    ...
    cairo_line_to(cr, graph->priv->box_x + graph->priv->box_width,
                  (int)b + 0.5f);
    cairo_stroke(cr);
  }
 ...
 **/
static void gpm_graph_widget_draw_labels(GpmGraphWidget *graph, cairo_t *cr) {
  guint i;
  gfloat b;
  gchar *text;
  gfloat value;
  ...
  for (i = 0; i < 11; i++) {
    ...
    pango_layout_get_pixel_extents(graph->priv->layout, &ink_rect,
                                   &logical_rect);
  ...
  for (i = 0; i < 11; i++) {
    b = graph->priv->box_y + ((gfloat)i * divheight);
    value =
        ((gfloat)length_y / 10.0f) * (10 - (gfloat)i) + graph->priv->start_y;
 ...
 * Draw the X and the Y labels onto the graph.
 **/
static guint gpm_graph_widget_get_y_label_max_width(GpmGraphWidget *graph,
                                                    cairo_t *cr) {
  guint i;
  gchar *text;
  gint value;
  ...
  for (i = 0; i < 11; i++) {
    ...
    pango_layout_get_pixel_extents(graph->priv->layout, &ink_rect,
                                   &logical_rect);
    if (ink_rect.width > (gint)biggest) biggest = ink_rect.width;
    g_free(text);
  }
  return biggest;
}
 ...
 **/
static void gpm_graph_widget_autorange_x(GpmGraphWidget *graph) {
  ...
  g_debug("Processed(1) range is %i<x<%i", graph->priv->start_x,
          graph->priv->stop_x);

  /* if percentage, and close to the end points, then extend */
  if (graph->priv->type_x == GPM_GRAPH_WIDGET_TYPE_PERCENTAGE) {
    if (graph->priv->stop_x >= 90) graph->priv->stop_x = 100;
  ...
  g_debug("Processed range is %i<x<%i", graph->priv->start_x,
          graph->priv->stop_x);
}
 ...
 * resolution but also a number that scales "well" to a 10x10 grid.
 **/
static void gpm_graph_widget_autorange_y(GpmGraphWidget *graph) {
  ...
  g_debug("Processed(1) range is %i<y<%i", graph->priv->start_y,
          graph->priv->stop_y);

  if (graph->priv->type_y == GPM_GRAPH_WIDGET_TYPE_PERCENTAGE) {
    if (graph->priv->stop_y >= 90) graph->priv->stop_y = 100;
    if (graph->priv->start_y > 0 && graph->priv->start_y <= 10)
      graph->priv->start_y = 0;
  } else if (graph->priv->type_y == GPM_GRAPH_WIDGET_TYPE_TIME) {
    if (graph->priv->start_y <= 60 * 10) graph->priv->start_y = 0;
  }

  g_debug("Processed range is %i<y<%i", graph->priv->start_y,
          graph->priv->stop_y);
}
 ...
 **/
static void gpm_graph_widget_set_color(cairo_t *cr, guint32 color) {
  guint8 r, g, b;
  egg_color_to_rgb(color, &r, &g, &b);
  cairo_set_source_rgb(cr, ((gdouble)r) / 256.0f, ((gdouble)g) / 256.0f,
                       ((gdouble)b) / 256.0f);
}
 ...
 **/
static void gpm_graph_widget_draw_legend_line(cairo_t *cr, gfloat x, gfloat y,
                                              guint32 color) {
  gfloat width = 10;
  gfloat height = 2;
  /* background */
  cairo_rectangle(cr, (int)(x - (width / 2)) + 0.5,
                  (int)(y - (height / 2)) + 0.5, width, height);
 ...
 **/
static void gpm_graph_widget_get_pos_on_graph(GpmGraphWidget *graph,
                                              gfloat data_x, gfloat data_y,
                                              float *x, float *y) {
  *x = graph->priv->box_x +
       (graph->priv->unit_x * (data_x - graph->priv->start_x)) + 1;
  *y = graph->priv->box_y +
       (graph->priv->unit_y * (gfloat)(graph->priv->stop_y - data_y)) + 1.5;
}

/**
 * gpm_graph_widget_draw_dot:
 **/
static void gpm_graph_widget_draw_dot(cairo_t *cr, gfloat x, gfloat y,
                                      guint32 color) {
  gfloat width;
  /* box */
  width = 2.0;
  cairo_rectangle(cr, (gint)x + 0.5f - (width / 2),
                  (gint)y + 0.5f - (width / 2), width, width);
  gpm_graph_widget_set_color(cr, color);
  cairo_fill(cr);
  cairo_rectangle(cr, (gint)x + 0.5f - (width / 2),
                  (gint)y + 0.5f - (width / 2), width, width);
 ...
 **/
static void gpm_graph_widget_draw_line(GpmGraphWidget *graph, cairo_t *cr) {
  gfloat oldx, oldy;
  gfloat newx, newy;
  GPtrArray *data;
  GPtrArray *array;
  ...
  for (j = 0; j < array->len; j++) {
    ...
    if (plot == GPM_GRAPH_WIDGET_PLOT_POINTS ||
        plot == GPM_GRAPH_WIDGET_PLOT_BOTH)
      gpm_graph_widget_draw_dot(cr, oldx, oldy, point->color);

    for (i = 1; i < data->len; i++) {
      point = (GpmPointObj *)g_ptr_array_index(data, i);

      gpm_graph_widget_get_pos_on_graph(graph, point->x, point->y, &newx,
                                        &newy);
      ...
      /* draw line */
      if (plot == GPM_GRAPH_WIDGET_PLOT_LINE ||
          plot == GPM_GRAPH_WIDGET_PLOT_BOTH) {
 ...
 **/
static void gpm_graph_widget_draw_bounding_box(cairo_t *cr, gint x, gint y,
 ...
 * @height: The item height
 **/
static void gpm_graph_widget_draw_legend(GpmGraphWidget *graph, gint x, gint y,
                                         gint width, gint height) {
 ...
 **/
static gboolean gpm_graph_widget_legend_calculate_size(GpmGraphWidget *graph,
                                                       cairo_t *cr,
                                                       guint *width,
                                                       guint *height) {
  guint i;
  ...
  *height = 0;
  ...
  for (i = 0; i < g_slist_length(graph->priv->key_data); i++) {
    ...
    *height = *height + GPM_GRAPH_WIDGET_LEGEND_SPACING;
    ...
    pango_layout_get_pixel_extents(graph->priv->layout, &ink_rect,
                                   &logical_rect);
    if ((gint)*width < ink_rect.width) *width = ink_rect.width;
  }
...
static gboolean gpm_graph_widget_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  gpm_graph_widget_legend_calculate_size(graph, cr, &legend_width,
                                         &legend_height);
  cairo_save(cr);

  /* we need this so we know the y text */
  if (graph->priv->autorange_x) gpm_graph_widget_autorange_x(graph);
  if (graph->priv->autorange_y) gpm_graph_widget_autorange_y(graph);
  ...
  if (graph->priv->use_legend && legend_height > 0) {
    graph->priv->box_width =
        allocation.width - (3 + legend_width + 5 + graph->priv->box_x);
    legend_x = graph->priv->box_x + graph->priv->box_width + 6;
    legend_y = graph->priv->box_y;
  } else {
    graph->priv->box_width = allocation.width - (3 + graph->priv->box_x);
  }

  /* graph background */
  gpm_graph_widget_draw_bounding_box(cr, graph->priv->box_x, graph->priv->box_y,
                                     graph->priv->box_width,
                                     graph->priv->box_height);
  if (graph->priv->use_grid) gpm_graph_widget_draw_grid(graph, cr);
  ...
  if (graph->priv->use_legend && legend_height > 0)
    gpm_graph_widget_draw_legend(graph, legend_x, legend_y, legend_width,
                                 legend_height);
