...
enum { SIGNAL_ALARM_EXPIRED, SIGNAL_RESET, LAST_SIGNAL };
 ...
 */
static void egg_idletime_xsync_alarm_set(EggIdletime *idletime,
...
void egg_idletime_alarm_reset_all(EggIdletime *idletime) {
  ...
  if (!idletime->priv->reset_set) return;
 ...
 */
static void egg_idletime_set_reset_alarm(EggIdletime *idletime,
 ...
 */
static EggIdletimeAlarm *egg_idletime_alarm_find_event(
    EggIdletime *idletime, XSyncAlarmNotifyEvent *alarm_event) {
  guint i;
  EggIdletimeAlarm *alarm;
  for (i = 0; i < idletime->priv->array->len; i++) {
    alarm = g_ptr_array_index(idletime->priv->array, i);
    if (alarm_event->alarm == alarm->xalarm) return alarm;
  }
  return NULL;
}
 ...
 */
static GdkFilterReturn egg_idletime_event_filter_cb(GdkXEvent *gdkxevent,
                                                    GdkEvent *event,
                                                    gpointer data) {
  EggIdletimeAlarm *alarm;
  ...
  if (alarm == NULL) return GDK_FILTER_CONTINUE;
 ...
 */
gboolean egg_idletime_alarm_set(EggIdletime *idletime, guint id,
                                guint timeout) {
  EggIdletimeAlarm *alarm;
 ...
 */
static gboolean egg_idletime_alarm_free(EggIdletime *idletime,
                                        EggIdletimeAlarm *alarm) {
  g_return_val_if_fail(EGG_IS_IDLETIME(idletime), FALSE);
  g_return_val_if_fail(alarm != NULL, FALSE);

  if (alarm->xalarm) XSyncDestroyAlarm(idletime->priv->dpy, alarm->xalarm);
...
static void egg_idletime_class_init(EggIdletimeClass *klass) {
  ...
  signals[SIGNAL_ALARM_EXPIRED] = g_signal_new(
      "alarm-expired", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EggIdletimeClass, alarm_expired), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);
  signals[SIGNAL_RESET] =
      g_signal_new("reset", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EggIdletimeClass, reset), NULL, NULL,
...
static void egg_idletime_init(EggIdletime *idletime) {
  ...
  /* get the sync event */
  if (!XSyncQueryExtension(idletime->priv->dpy, &idletime->priv->sync_event,
                           &sync_error)) {
...
static void gpm_alarm_expired_cb(EggIdletime *idletime, guint alarm,
                                 gpointer data) {
  last_alarm = alarm;
...
static void wait_until_alarm(void) {
  ...
  g_print("*****************************\n");
  while (last_alarm == 0) g_main_context_iteration(NULL, FALSE);
}

static void wait_until_reset(void) {
  if (last_alarm == 0) return;
  g_print("*****************************\n");
...
void egg_idletime_test(gpointer data) {
  ...
  if (egg_test_start(test, "EggIdletime") == FALSE) return;
  ...
  g_signal_connect(idletime, "alarm-expired", G_CALLBACK(gpm_alarm_expired_cb),
                   NULL);
