...
enum { BRIGHTNESS_CHANGED, LAST_SIGNAL };
 ...
 **/
static gboolean gpm_brightness_helper_strtoint(const gchar *text, gint *value) {
  gchar *endptr = NULL;
  gint64 value_raw;
...
static gint gpm_brightness_helper_get_value(const gchar *argument) {
  ...
  /* get the data */
  command =
      g_strdup_printf(SBINDIR "/mate-power-backlight-helper --%s", argument);
  ret = g_spawn_command_line_sync(command, &stdout_data, NULL, &exit_status,
                                  &error);
  if (!ret) {
 ...
 **/
static gboolean gpm_brightness_helper_set_value(const gchar *argument,
                                                gint value) {
  gboolean ret;
  GError *error = NULL;
  gint exit_status = 0;
  ...
  /* get the data */
  command =
      g_strdup_printf("pkexec " SBINDIR "/mate-power-backlight-helper --%s %i",
                      argument, value);
 ...
 **/
static gboolean gpm_brightness_output_get_internal(GpmBrightness *brightness,
                                                   RROutput output,
                                                   guint *cur) {
 ...
 **/
static gboolean gpm_brightness_output_set_internal(GpmBrightness *brightness,
                                                   RROutput output,
                                                   guint value) {
  GdkDisplay *display;
  ...
  if (ret) brightness->priv->hw_changed = TRUE;
  return ret;
}
 ...
 **/
static gboolean gpm_brightness_output_get_limits(GpmBrightness *brightness,
                                                 RROutput output, guint *min,
                                                 guint *max) {
  ...
  info = XRRQueryOutputProperty(brightness->priv->dpy, output,
                                brightness->priv->backlight);
  if (info == NULL) {
 ...
 **/
static gboolean gpm_brightness_output_get_percentage(GpmBrightness *brightness,
                                                     RROutput output) {
  guint cur;
  gboolean ret;
  guint min, max;
 ...
 **/
static gboolean gpm_brightness_output_down(GpmBrightness *brightness,
                                           RROutput output) {
  guint cur;
  guint step;
  gboolean ret;
 ...
 **/
static gboolean gpm_brightness_output_up(GpmBrightness *brightness,
                                         RROutput output) {
  guint cur;
  gboolean ret;
  guint min, max;
 ...
 **/
static gboolean gpm_brightness_output_set(GpmBrightness *brightness,
                                          RROutput output) {
  guint cur;
  gboolean ret;
  guint min, max;
  ...
  shared_value_abs = egg_discrete_from_percent(brightness->priv->shared_value,
                                               (max - min) + 1);
  g_debug("percent=%i, absolute=%i", brightness->priv->shared_value,
          shared_value_abs);

  g_debug("hard value=%i, min=%i, max=%i", cur, min, max);
  if (shared_value_abs > (gint)max) shared_value_abs = max;
  if (shared_value_abs < (gint)min) shared_value_abs = min;
 ...
 **/
static gboolean gpm_brightness_foreach_resource(GpmBrightness *brightness,
                                                GpmXRandROp op,
 ...
 **/
static gboolean gpm_brightness_foreach_screen(GpmBrightness *brightness,
                                              GpmXRandROp op) {
  guint i;
  guint length;
  ...
  if (!brightness->priv->has_extension) return FALSE;

  /* do for each screen */
  length = brightness->priv->resources->len;
  for (i = 0; i < length; i++) {
    resource =
        (XRRScreenResources *)g_ptr_array_index(brightness->priv->resources, i);
    g_debug("using resource %p", resource);
    ret = gpm_brightness_foreach_resource(brightness, op, resource);
    if (ret) success_any = TRUE;
  }
...
static gboolean gpm_brightness_trust_cache(GpmBrightness *brightness) {
  g_return_val_if_fail(GPM_IS_BRIGHTNESS(brightness), FALSE);
  /* only return the cached value if the cache is trusted and we have change
   * events */
  if (brightness->priv->cache_trusted && brightness->priv->has_changed_events) {
    g_debug("using cache for value %u (okay)",
            brightness->priv->cache_percentage);
    return TRUE;
  }

  /* can we trust that if we set a value 5 minutes ago, will it still be valid
   * now? if we have multiple things setting policy on the workstation, e.g.
   * fast user switching or kpowersave, then this will be invalid -- this logic
   * may be insane */
  if (GPM_SOLE_SETTER_USE_CACHE && brightness->priv->cache_trusted) {
    g_debug("using cache for value %u (probably okay)",
            brightness->priv->cache_percentage);
    return TRUE;
  }
  return FALSE;
 ...
 **/
gboolean gpm_brightness_set(GpmBrightness *brightness, guint percentage,
                            gboolean *hw_changed) {
  gboolean ret = FALSE;
  ...
  if (!ret) {
    ...
    brightness->priv->extension_current = egg_discrete_from_percent(
        percentage, brightness->priv->extension_levels + 1);
    ret = gpm_brightness_helper_set_value("set-brightness",
                                          brightness->priv->extension_current);
  }

  /* did the hardware have to be modified? */
  if (ret && hw_changed != NULL) *hw_changed = brightness->priv->hw_changed;

  /* we did something to the hardware, so untrusted */
  if (ret) brightness->priv->cache_trusted = FALSE;
 ...
 **/
gboolean gpm_brightness_up(GpmBrightness *brightness, gboolean *hw_changed) {
  gboolean ret = FALSE;
  guint step;
  ...
  if (!ret) {
    ...
    if (brightness->priv->extension_current <
      ...
      if (brightness->priv->extension_current >
        ...
        brightness->priv->extension_current =
            brightness->priv->extension_levels;
      ret = gpm_brightness_helper_set_value(
          "set-brightness", brightness->priv->extension_current);
    }
    if (hw_changed != NULL) *hw_changed = ret;
    brightness->priv->cache_trusted = FALSE;
    goto out;
  }
out:
 ...
 **/
gboolean gpm_brightness_down(GpmBrightness *brightness, gboolean *hw_changed) {
  gboolean ret = FALSE;
  guint step;
 ...
 **/
static GdkFilterReturn gpm_brightness_filter_xevents(GdkXEvent *xevent,
                                                     GdkEvent *event,
                                                     gpointer data) {
  GpmBrightness *brightness = GPM_BRIGHTNESS(data);
  if (event->type == GDK_NOTHING) return GDK_FILTER_CONTINUE;
 ...
 **/
static void gpm_brightness_monitors_changed(GdkScreen *screen,
...
static void gpm_brightness_update_cache(GpmBrightness *brightness) {
  ...
  length = brightness->priv->resources->len;
  if (length > 0) g_ptr_array_set_size(brightness->priv->resources, 0);
...
gboolean gpm_brightness_has_hw(GpmBrightness *brightness) {
  ...
  /* use XRandR first */
  if (brightness->priv->has_extension) return TRUE;

  /* fallback to legacy access */
  if (brightness->priv->extension_levels < 0)
    brightness->priv->extension_levels =
        gpm_brightness_helper_get_value("get-max-brightness");
  if (brightness->priv->extension_levels > 0) return TRUE;
  return FALSE;
}
...
static void gpm_brightness_class_init(GpmBrightnessClass *klass) {
  ...
  signals[BRIGHTNESS_CHANGED] = g_signal_new(
      "brightness-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmBrightnessClass, brightness_changed), NULL, NULL,
...
static void gpm_brightness_init(GpmBrightness *brightness) {
  ...
  brightness->priv->resources =
      g_ptr_array_new_with_free_func((GDestroyNotify)XRRFreeScreenResources);

  /* can we do this */
  brightness->priv->has_extension = gpm_brightness_setup_display(brightness);
  if (brightness->priv->has_extension == FALSE) g_debug("no XRANDR extension");
  ...
  if (!XRRQueryExtension(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
                         &event_base, &ignore)) {
  ...
  XRRSelectInput(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
                 ...
                 RRScreenChangeNotifyMask |
                     RROutputPropertyNotifyMask); /* <--- the only one we need,
                                                     but see rh:345551 */
  gdk_display_flush(display);
  if (gdk_x11_display_error_trap_pop(display))
