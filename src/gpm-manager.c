...
GQuark gpm_manager_error_quark(void) {
  ...
  if (!quark) quark = g_quark_from_static_string("gpm_manager_error");
  return quark;
}
...
static gboolean gpm_manager_play_loop_timeout_cb(GpmManager *manager) {
  ...
  ca_context_play_full(context, 0, manager->priv->critical_alert_loop_props,
                       NULL, NULL);
  return TRUE;
}
 ...
 **/
static gboolean gpm_manager_play_loop_start(GpmManager *manager,
                                            GpmManagerSound action,
  ...
  ca_proplist_sets(manager->priv->critical_alert_loop_props, CA_PROP_EVENT_ID,
                   id);
  ca_proplist_sets(manager->priv->critical_alert_loop_props,
                   CA_PROP_EVENT_DESCRIPTION, desc);

  manager->priv->critical_alert_timeout_id = g_timeout_add_seconds(
      timeout, (GSourceFunc)gpm_manager_play_loop_timeout_cb, manager);
  ...
  context = ca_gtk_context_get_for_screen(gdk_screen_get_default());
  retval = ca_context_play(context, 0, CA_PROP_EVENT_ID, id,
                           CA_PROP_EVENT_DESCRIPTION, desc, NULL);
  if (retval < 0) g_warning("failed to play %s: %s", id, ca_strerror(retval));
  return TRUE;
}
 ...
 * gpm_manager_play:
 **/
static gboolean gpm_manager_play(GpmManager *manager, GpmManagerSound action,
                                 gboolean force) {
  const gchar *id = NULL;
  const gchar *desc = NULL;
  gboolean ret;
 ...
 **/
static gboolean gpm_manager_is_inhibit_valid(GpmManager *manager,
...
static void gpm_manager_sync_policy_sleep(GpmManager *manager) {
  ...
  if (!manager->priv->on_battery) {
    sleep_computer = g_settings_get_int(manager->priv->settings,
                                        GPM_SETTINGS_SLEEP_COMPUTER_AC);
    sleep_display = g_settings_get_int(manager->priv->settings,
 ...
 **/
static void gpm_manager_notification_closed_cb(
    NotifyNotification *notification, NotifyNotification **notification_class) {
 ...
 * gpm_manager_notify:
 **/
static gboolean gpm_manager_notify(GpmManager *manager,
  ...
  if (manager->priv->status_icon != NULL &&
    ...
    notification = notify_notification_new(
        title, message,
        gtk_status_icon_get_icon_name(manager->priv->status_icon));
  else
  ...
  if (!ret) {
    ...
    dialog = gtk_message_dialog_new_with_markup(
        NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
        GTK_BUTTONS_CLOSE, "<span size='larger'><b>%s</b></span>", title);
 ...
 **/
static void gpm_manager_sleep_failure_response_cb(GtkDialog *dialog,
                                                  gint response_id,
                                                  GpmManager *manager) {
  ...
  if (response_id == GTK_RESPONSE_HELP) {
    uri = g_settings_get_string(manager->priv->settings,
                                GPM_SETTINGS_NOTIFY_SLEEP_FAILED_URI);
    ret = gtk_show_uri_on_window(GTK_WINDOW(dialog), uri,
 ...
 **/
static void gpm_manager_sleep_failure(GpmManager *manager, gboolean is_suspend,
                                      const gchar *detail) {
  ...
  show_sleep_failed = g_settings_get_boolean(manager->priv->settings,
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gpm_manager_sleep_failure_response_cb), manager);
out:
  g_free(uri);
 ...
 **/
static gboolean gpm_manager_action_suspend(GpmManager *manager,
                                           const gchar *reason) {
  gboolean ret;
  GError *error = NULL;
 ...
 **/
static gboolean gpm_manager_action_hibernate(GpmManager *manager,
                                             const gchar *reason) {
  gboolean ret;
  GError *error = NULL;
 ...
 * @reason: The reason we are performing the policy action, e.g. "battery
 *critical"
 *
 * Does one of the policy actions specified in the settings.
 **/
static gboolean gpm_manager_perform_policy(GpmManager *manager,
...
static void gpm_manager_idle_do_sleep(GpmManager *manager) {
  ...
  if (policy == GPM_ACTION_POLICY_NOTHING) {
    ...
    ret = gpm_control_suspend(manager->priv->control, &error);
    if (!ret) {
      g_warning("cannot suspend (error: %s), so trying hibernate",
                error->message);
      g_error_free(error);
      error = NULL;
      ret = gpm_control_hibernate(manager->priv->control, &error);
    ...
    ret = gpm_control_hibernate(manager->priv->control, &error);
    if (!ret) {
      g_warning("cannot hibernate (error: %s), so trying suspend",
                error->message);
      g_error_free(error);
      error = NULL;
      ret = gpm_control_suspend(manager->priv->control, &error);
 ...
 * @manager: This class instance
 **/
static void gpm_manager_button_pressed_cb(GpmButton *button, const gchar *type,
                                          GpmManager *manager) {
  gchar *message;
  ...
  if (g_strcmp0(type, GPM_BUTTON_POWER) == 0) {
    ...
    gpm_manager_notify(manager, &manager->priv->notification_general,
                       _("Power Information"), message,
                       GPM_MANAGER_NOTIFY_TIMEOUT_LONG, "dialog-information",
 ...
 **/
static void gpm_manager_client_changed_cb(UpClient *client, GParamSpec *pspec,
                                          GpmManager *manager) {
  ...
  g_object_get(client, "on-battery", &on_battery, "lid-is-closed",
               &lid_is_closed, NULL);
  ...
  event_when_closed = g_settings_get_boolean(manager->priv->settings,
 ...
 **/
static void gpm_manager_settings_changed_cb(GSettings *settings,
                                            const gchar *key,
                                            GpmManager *manager) {
 ...
 */
static void gpm_manager_engine_icon_changed_cb(GpmEngine *engine, gchar *icon,
                                               GpmManager *manager) {
 ...
 */
static void gpm_manager_engine_summary_changed_cb(GpmEngine *engine,
                                                  gchar *summary,
                                                  GpmManager *manager) {
 ...
 */
static void gpm_manager_engine_low_capacity_cb(GpmEngine *engine,
                                               UpDevice *device,
                                               GpmManager *manager) {
  gchar *message = NULL;
  const gchar *title;
  ...
  message = g_strdup_printf(_("Battery has a very low capacity (%1.1f%%), "
                            ...
                            capacity);
  gpm_manager_notify(manager, &manager->priv->notification_general, title,
                     message, GPM_MANAGER_NOTIFY_TIMEOUT_SHORT,
 ...
 */
static void gpm_manager_engine_fully_charged_cb(GpmEngine *engine,
                                                UpDevice *device,
                                                GpmManager *manager) {
  UpDeviceKind kind;
  ...
  if (kind == UP_DEVICE_KIND_BATTERY) {
    ...
    if (g_str_has_prefix(native_path, "dummy")) plural = 2;
 ...
 */
static void gpm_manager_engine_discharging_cb(GpmEngine *engine,
                                              UpDevice *device,
                                              GpmManager *manager) {
  UpDeviceKind kind;
  gboolean ret;
  ...
  /* get device properties */
  g_object_get(device, "kind", &kind, "percentage", &percentage,
               "time-to-empty", &time_to_empty, NULL);

  /* only show text if there is a valid time */
  if (time_to_empty > 0) remaining_text = gpm_get_timestring(time_to_empty);
  ...
  if (kind == UP_DEVICE_KIND_BATTERY) {
    ...
    if (remaining_text != NULL) {
      ...
      message = g_strdup_printf(_("%s of battery power remaining (%.0f%%)"),
                                remaining_text, percentage);
    } else {
      /* TRANSLATORS: the device is discharging, but we only have a percentage
       */
      message =
          g_strdup_printf(_("%s discharging (%.0f%%)"), kind_desc, percentage);
    }
  ...
  gpm_manager_notify(manager, &manager->priv->notification_discharging, title,
                     message, GPM_MANAGER_NOTIFY_TIMEOUT_LONG, icon,
                     NOTIFY_URGENCY_NORMAL);
out:
  g_free(icon);
 ...
 */
static gboolean gpm_manager_engine_just_laptop_battery(GpmManager *manager) {
  UpDevice *device;
  UpDeviceKind kind;
  GPtrArray *array;
  gboolean ret = TRUE;
 ...
 */
static void gpm_manager_engine_charge_low_cb(GpmEngine *engine,
                                             UpDevice *device,
                                             GpmManager *manager) {
  ...
  if (kind == UP_DEVICE_KIND_BATTERY) {
    ...
    if (ret) {
      ...
      /* TRANSLATORS: laptop battery low, and we have more than one kind of
       * battery */
    ...
    /* TRANSLATORS: tell the user how much time they have got */
    message = g_strdup_printf(_("Approximately <b>%s</b> remaining (%.0f%%)"),
                              remaining_text, percentage);
    ...
    message = g_strdup_printf(
        _("Approximately <b>%s</b> of remaining UPS backup power (%.0f%%)"),
        remaining_text, percentage);
  } else if (kind == UP_DEVICE_KIND_MOUSE) {
    gboolean notify = g_settings_get_boolean(
        manager->priv->settings, GPM_SETTINGS_NOTIFY_LOW_CAPACITY_MOUSE);
    if (!notify) goto out;
    ...
    message = g_strdup_printf(_("Wireless mouse is low in power (%.0f%%)"),
                              percentage);
    ...
    /* TRANSLATORS: tell user more details */
    message =
        g_strdup_printf(_("Cell phone is low in power (%.0f%%)"), percentage);
    ...
    /* TRANSLATORS: tell user more details */
    message =
        g_strdup_printf(_("Media player is low in power (%.0f%%)"), percentage);
  ...
  /* get correct icon */
  icon = gpm_upower_get_device_icon(device);
  gpm_manager_notify(manager, &manager->priv->notification_warning_low, title,
                     message, GPM_MANAGER_NOTIFY_TIMEOUT_LONG, icon,
                     NOTIFY_URGENCY_NORMAL);
 ...
 */
static void gpm_manager_engine_charge_critical_cb(GpmEngine *engine,
                                                  UpDevice *device,
                                                  GpmManager *manager) {
  const gchar *title = NULL;
  gchar *message = NULL;
  ...
  if (kind == UP_DEVICE_KIND_BATTERY) {
    ...
    if (ret) {
      ...
      /* TRANSLATORS: laptop battery critically low, and we have more than one
       * type of battery */
    ...
    if (policy == GPM_ACTION_POLICY_NOTHING) {
      ...
      message = g_strdup_printf(
          _("Computer will suspend very soon unless it is plugged in."));

    } else if (policy == GPM_ACTION_POLICY_HIBERNATE) {
      /* TRANSLATORS: give the user a ultimatum */
      message = g_strdup_printf(
          _("Computer will hibernate very soon unless it is plugged in."));

    } else if (policy == GPM_ACTION_POLICY_SHUTDOWN) {
      /* TRANSLATORS: give the user a ultimatum */
      message = g_strdup_printf(
          _("Computer will shutdown very soon unless it is plugged in."));
    }
    ...
    /* TRANSLATORS: give the user a ultimatum */
    message = g_strdup_printf(
        _("Approximately <b>%s</b> of remaining UPS power (%.0f%%). "
    ...
    message = g_strdup_printf(
        _("Wireless mouse is very low in power (%.0f%%). "
    ...
    /* TRANSLATORS: the device is just going to stop working */
    message = g_strdup_printf(
        _("Wireless keyboard is very low in power (%.0f%%). "
          "This device will soon stop functioning if not charged."),
        percentage);
  } else if (kind == UP_DEVICE_KIND_PDA) {
    ...
    /* TRANSLATORS: the device is just going to stop working */
    message = g_strdup_printf(
        _("Cell phone is very low in power (%.0f%%). "
          "This device will soon stop functioning if not charged."),
        percentage);
    ...
    /* TRANSLATORS: the device is just going to stop working */
    message = g_strdup_printf(
        _("Media player is very low in power (%.0f%%). "
          "This device will soon stop functioning if not charged."),
        percentage);
  } else if (kind == UP_DEVICE_KIND_TABLET) {
    /* TRANSLATORS: the cell battery is very low */
    ...
    /* TRANSLATORS: the device is just going to stop working */
    message = g_strdup_printf(
        _("Tablet is very low in power (%.0f%%). "
          "This device will soon stop functioning if not charged."),
        percentage);
  } else if (kind == UP_DEVICE_KIND_COMPUTER) {
    /* TRANSLATORS: the cell battery is very low */
    ...
    /* TRANSLATORS: the device is just going to stop working */
    message =
        g_strdup_printf(_("Attached computer is very low in power (%.0f%%). "
  ...
  icon = gpm_upower_get_device_icon(device);
  gpm_manager_notify(manager, &manager->priv->notification_warning_low, title,
                     message, GPM_MANAGER_NOTIFY_TIMEOUT_NEVER, icon,
  ...
  switch (kind) {
    ...
    case UP_DEVICE_KIND_UPS:
      ...
      gpm_manager_play_loop_start(manager, GPM_MANAGER_SOUND_BATTERY_LOW, TRUE,
 ...
 */
static void gpm_manager_engine_charge_action_cb(GpmEngine *engine,
                                                UpDevice *device,
                                                GpmManager *manager) {
  const gchar *title = NULL;
  gchar *message = NULL;
  ...
  if (kind == UP_DEVICE_KIND_BATTERY) {
    ...
    if (policy == GPM_ACTION_POLICY_NOTHING) {
      ...
      message =
          g_strdup(_("The battery is below the critical level and "
    ...
    timer_id = g_timeout_add_seconds(
        20, (GSourceFunc)manager_critical_action_do, manager);
    ...
    if (policy == GPM_ACTION_POLICY_NOTHING) {
      /* TRANSLATORS: computer will shutdown without saving data */
      message =
          g_strdup(_("The UPS is below the critical level and "
                     "this computer will <b>power-off</b> when the "
 ...
 **/
static void gpm_manager_control_resume_cb(GpmControl *control,
...
static gint32 gpm_manager_systemd_inhibit(GDBusProxy *proxy) {
  ...
  res = g_dbus_proxy_call_with_unix_fd_list_sync(
      proxy, "Inhibit",
      g_variant_new("(ssss)", arg_what, arg_who, arg_why, arg_mode),
...
static void on_icon_theme_change(GtkSettings *settings, GParamSpec *pspec,
                                 GpmManager *manager) {
...
static void gpm_manager_init(GpmManager *manager) {
  ...
  /* We want to inhibit the systemd suspend options, and take care of them
   * ourselves */
  if (LOGIND_RUNNING()) {
  ...
  g_object_get(manager->priv->client, "on-battery", &manager->priv->on_battery,
               NULL);
