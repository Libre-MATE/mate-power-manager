 ...
 **/
static UpDeviceLevel gpm_engine_get_warning(GpmEngine *engine,
                                            UpDevice *device) {
...
gchar *gpm_engine_get_summary(GpmEngine *engine) {
  ...
  for (i = 0; i < array->len; i++) {
    ...
    g_object_get(device, "is-present", &is_present, "state", &state, NULL);
    if (!is_present) continue;
    if (state == UP_DEVICE_STATE_EMPTY) continue;
    part = gpm_upower_get_device_summary(device);
    if (part != NULL) g_string_append_printf(tooltip, "%s\n", part);
    g_free(part);
  }
 ...
 * Returns the icon
 **/
static gchar *gpm_engine_get_icon_priv(GpmEngine *engine,
  ...
  for (i = 0; i < array->len; i++) {
    ...
    g_object_get(device, "kind", &kind, "state", &state, "is-present",
                 &is_present, NULL);
    ...
    warning_temp = GPOINTER_TO_INT(
        g_object_get_data(G_OBJECT(device), "engine-warning-old"));
    if (kind == device_kind && is_present) {
      if (warning != GPM_ENGINE_WARNING_NONE) {
        if (warning_temp == warning) return gpm_upower_get_device_icon(device);
        continue;
      }
      if (use_state) {
...
gchar *gpm_engine_get_icon(GpmEngine *engine) {
  ...
  icon = gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_BATTERY,
                                  GPM_ENGINE_WARNING_CRITICAL, FALSE);
  if (icon != NULL) return icon;
  icon = gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_UPS,
                                  GPM_ENGINE_WARNING_CRITICAL, FALSE);
  if (icon != NULL) return icon;
  icon = gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_MOUSE,
                                  GPM_ENGINE_WARNING_CRITICAL, FALSE);
  if (icon != NULL) return icon;
  icon = gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_KEYBOARD,
                                  GPM_ENGINE_WARNING_CRITICAL, FALSE);
  if (icon != NULL) return icon;
...
static gboolean gpm_engine_recalculate_state_icon(GpmEngine *engine) {
  ...
  if (icon == NULL) {
    ...
    if (engine->priv->previous_icon == NULL) return FALSE;
 ...
 **/
static void gpm_engine_settings_key_changed_cb(GSettings *settings,
                                               const gchar *key,
                                               GpmEngine *engine) {
 ...
 **/
static gboolean gpm_engine_device_check_capacity(GpmEngine *engine,
                                                 UpDevice *device) {
  gboolean ret;
  UpDeviceKind kind;
  gdouble capacity;
  ...
  /* not laptop battery */
  if (kind != UP_DEVICE_KIND_BATTERY) return FALSE;

  /* capacity okay */
  if (capacity > 50.0f) return FALSE;

  /* capacity invalid */
  if (capacity < 1.0f) return FALSE;

  /* only emit this if specified in the settings */
  ret = g_settings_get_boolean(engine->priv->settings,
...
static void gpm_engine_device_add(GpmEngine *engine, UpDevice *device) {
  ...
  g_debug("adding %s with state %s", up_device_get_object_path(device),
  ...
  g_signal_connect(device, "notify", G_CALLBACK(gpm_engine_device_changed_cb),
                   engine);
 ...
 **/
static void gpm_engine_device_added_cb(UpClient *client, UpDevice *device,
                                       GpmEngine *engine) {
  gpm_engine_device_add(engine, device);
}
 ...
 **/
static void gpm_engine_device_removed_cb(UpClient *client,
...
static void gpm_engine_device_changed_cb(UpDevice *device, GParamSpec *pspec,
  ...
  g_debug("%s state is now %s", up_device_get_object_path(device),
          up_device_state_to_string(state));

  /* see if any interesting state changes have happened */
  state_old =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(device), "engine-state-old"));
  if (state_old != state) {
...
UpDevice *gpm_engine_get_primary_device(GpmEngine *engine) {
  ...
  for (i = 0; i < engine->priv->array->len; i++) {
    ...
    /* get device properties */
    g_object_get(device_tmp, "kind", &kind, "state", &state, "is-present",
                 &is_present, NULL);
    ...
    /* not discharging */
    if (state != UP_DEVICE_STATE_DISCHARGING) continue;

    /* not battery */
    if (kind != UP_DEVICE_KIND_BATTERY) continue;
 ...
 **/
static void phone_device_added_cb(GpmPhone *phone, guint idx,
                                  GpmEngine *engine) {
  UpDevice *device;
  ...
  /* get device properties */
  g_object_set(device, "kind", UP_DEVICE_KIND_PHONE, "is-rechargeable", TRUE,
 ...
 **/
static void phone_device_removed_cb(GpmPhone *phone, guint idx,
                                    GpmEngine *engine) {
  guint i;
  UpDevice *device;
  UpDeviceKind kind;
 ...
 **/
static void phone_device_refresh_cb(GpmPhone *phone, guint idx,
                                    GpmEngine *engine) {
  guint i;
  UpDevice *device;
  UpDeviceKind kind;
  ...
  for (i = 0; i < engine->priv->array->len; i++) {
    ...
    g_object_get(device, "kind", &kind, "state", &state, "percentage",
                 &percentage, "is-present", &is_present, NULL);

    if (kind == UP_DEVICE_KIND_PHONE) {
      is_present = gpm_phone_get_present(phone, idx);
      state = gpm_phone_get_on_ac(phone, idx) ? UP_DEVICE_STATE_CHARGING
...
static void gpm_engine_init(GpmEngine *engine) {
  ...
  /* do we want to display the icon in the tray */
  engine->priv->icon_policy =
      g_settings_get_enum(engine->priv->settings, GPM_SETTINGS_ICON_POLICY);

  /* get percentage policy */
  engine->priv->low_percentage =
      g_settings_get_int(engine->priv->settings, GPM_SETTINGS_PERCENTAGE_LOW);
  engine->priv->critical_percentage = g_settings_get_int(
      engine->priv->settings, GPM_SETTINGS_PERCENTAGE_CRITICAL);
  engine->priv->action_percentage = g_settings_get_int(
      engine->priv->settings, GPM_SETTINGS_PERCENTAGE_ACTION);

  /* get time policy */
  engine->priv->low_time =
      g_settings_get_int(engine->priv->settings, GPM_SETTINGS_TIME_LOW);
  engine->priv->critical_time =
      g_settings_get_int(engine->priv->settings, GPM_SETTINGS_TIME_CRITICAL);
  engine->priv->action_time =
      g_settings_get_int(engine->priv->settings, GPM_SETTINGS_TIME_ACTION);

  /* we can disable this if the time remaining is inaccurate or just plain wrong
   */
  engine->priv->use_time_primary = g_settings_get_boolean(
      engine->priv->settings, GPM_SETTINGS_USE_TIME_POLICY);
...
static void gpm_engine_class_init(GpmEngineClass *klass) {
  ...
  signals[ICON_CHANGED] = g_signal_new(
      "icon-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, icon_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  signals[SUMMARY_CHANGED] = g_signal_new(
      "summary-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, summary_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  signals[LOW_CAPACITY] = g_signal_new(
      "low-capacity", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, low_capacity), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[FULLY_CHARGED] = g_signal_new(
      "fully-charged", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, fully_charged), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[DISCHARGING] = g_signal_new(
      "discharging", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, discharging), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[CHARGE_ACTION] = g_signal_new(
      "charge-action", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, charge_action), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[CHARGE_LOW] = g_signal_new(
      "charge-low", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, charge_low), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[CHARGE_CRITICAL] = g_signal_new(
      "charge-critical", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, charge_critical), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[DEVICES_CHANGED] = g_signal_new(
      "devices-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmEngineClass, devices_changed), NULL, NULL,
