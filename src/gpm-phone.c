...
enum { DEVICE_ADDED, DEVICE_REMOVED, DEVICE_REFRESH, LAST_SIGNAL };
 ...
 */
static void gpm_phone_battery_state_changed(DBusGProxy *proxy, guint idx,
 ...
 */
static void gpm_phone_num_batteries_changed(DBusGProxy *proxy, guint number,
                                            GpmPhone *phone) {
  g_return_if_fail(GPM_IS_PHONE(phone));
...
static void gpm_phone_class_init(GpmPhoneClass *klass) {
  ...
  signals[DEVICE_ADDED] = g_signal_new(
      "device-added", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmPhoneClass, device_added), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);

  signals[DEVICE_REMOVED] = g_signal_new(
      "device-removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmPhoneClass, device_removed), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);

  signals[DEVICE_REFRESH] = g_signal_new(
      "device-refresh", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GpmPhoneClass, device_refresh), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);
}
 ...
 */
static void gpm_phone_service_appeared_cb(GDBusConnection *connection,
                                          const gchar *name,
  ...
  if (phone->priv->proxy == NULL) {
    g_debug("get proxy");
    g_clear_error(&error);
    phone->priv->proxy = dbus_g_proxy_new_for_name_owner(
        phone->priv->connection, MATE_PHONE_MANAGER_DBUS_SERVICE,
        MATE_PHONE_MANAGER_DBUS_PATH, MATE_PHONE_MANAGER_DBUS_INTERFACE,
        &error);
    if (error != NULL) {
 ...
 */
static void gpm_phone_service_vanished_cb(GDBusConnection *connection,
...
static void gpm_phone_init(GpmPhone *phone) {
  ...
  phone->priv->watch_id = g_bus_watch_name(
      G_BUS_TYPE_SESSION, MATE_PHONE_MANAGER_DBUS_SERVICE,
      G_BUS_NAME_WATCHER_FLAGS_NONE,
      (GBusNameAppearedCallback)gpm_phone_service_appeared_cb,
      (GBusNameVanishedCallback)gpm_phone_service_vanished_cb, phone, NULL);
}
...
static void gpm_phone_finalize(GObject *object) {
  ...
  if (phone->priv->proxy != NULL) g_object_unref(phone->priv->proxy);
...
static void phone_device_refresh_cb(GpmPhone *phone, guint idx,
  ...
  if (idx == 0 && GPOINTER_TO_UINT(data) == 44) test_got_refresh = TRUE;
}
...
void gpm_phone_test(gpointer data) {
  ...
  if (egg_test_start(test, "GpmPhone") == FALSE) return;
  ...
  /* connect signals */
  g_signal_connect(phone, "device-refresh", G_CALLBACK(phone_device_refresh_cb),
                   GUINT_TO_POINTER(44));
  ...
  if (value == 1) {
    ...
    egg_test_failed(test, "not connected with %i (phone not connected?)",
                    value);
  }
