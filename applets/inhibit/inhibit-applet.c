...
#define GPM_INHIBIT_APPLET(o) \
  (G_TYPE_CHECK_INSTANCE_CAST((o), GPM_TYPE_INHIBIT_APPLET, GpmInhibitApplet))
#define GPM_INHIBIT_APPLET_CLASS(k) \
  (G_TYPE_CHECK_CLASS_CAST((k), GPM_TYPE_INHIBIT_APPLET, GpmInhibitAppletClass))
#define GPM_IS_INHIBIT_APPLET(o) \
  (G_TYPE_CHECK_INSTANCE_TYPE((o), GPM_TYPE_INHIBIT_APPLET))
#define GPM_IS_INHIBIT_APPLET_CLASS(k) \
  (G_TYPE_CHECK_CLASS_TYPE((k), GPM_TYPE_INHIBIT_APPLET))
#define GPM_INHIBIT_APPLET_GET_CLASS(o)                    \
  (G_TYPE_INSTANCE_GET_CLASS((o), GPM_TYPE_INHIBIT_APPLET, \
...
static void gpm_applet_size_allocate_cb(GtkWidget *widget,
                                        GdkRectangle *allocation);
;
static void gpm_applet_update_tooltip(GpmInhibitApplet *applet);
static gboolean gpm_applet_click_cb(GpmInhibitApplet *applet,
...
#define GPM_INHIBIT_APPLET_DESC \
  _("Allows user to inhibit automatic power saving.")
#define MATE_PANEL_APPLET_VERTICAL(p)        \
  (((p) == MATE_PANEL_APPLET_ORIENT_LEFT) || \
   ((p) == MATE_PANEL_APPLET_ORIENT_RIGHT))

/** cookie is returned as an unsigned integer */
static gboolean gpm_applet_inhibit(GpmInhibitApplet *applet,
  ...
  ret = dbus_g_proxy_call(applet->proxy, "Inhibit", &error, G_TYPE_STRING,
                          appname, G_TYPE_UINT, 0, /* xid */
                          G_TYPE_STRING, reason, G_TYPE_UINT,
                          1 + 2 + 4 + 8, /* logoff, switch, suspend, and idle */
                          G_TYPE_INVALID, G_TYPE_UINT, cookie, G_TYPE_INVALID);
  if (error) {
...
static gboolean gpm_applet_uninhibit(GpmInhibitApplet *applet, guint cookie) {
  GError *error = NULL;
  gboolean ret;
  ...
  ret = dbus_g_proxy_call(applet->proxy, "Uninhibit", &error, G_TYPE_UINT,
...
static void gpm_applet_update_icon(GpmInhibitApplet *applet) {
  ...
  gtk_image_set_from_icon_name(GTK_IMAGE(applet->image), icon,
...
static void gpm_applet_size_allocate_cb(GtkWidget *widget,
  ...
  /* Scale to the actual size of the applet, don't quantize to original icon
   * size */
...
static gboolean gpm_applet_click_cb(GpmInhibitApplet *applet,
  ...
  if (applet->cookie > 0) {
    ...
    g_debug("inhibiting");
    gpm_applet_inhibit(applet, GPM_INHIBIT_APPLET_NAME, _("Manual inhibit"),
                       &(applet->cookie));
  }
  /* update icon */
...
static void gpm_applet_dialog_about_cb(GtkAction *action, gpointer data) {
  ...
  gtk_show_about_dialog(
      NULL, "program-name", GPM_INHIBIT_APPLET_NAME, "version", VERSION,
      ...
      _("translator-credits"), "wrap-license", TRUE, "website", PACKAGE_URL,
      NULL);
 ...
 **/
static void gpm_inhibit_applet_class_init(GpmInhibitAppletClass *class) {
  /* nothing to do here */
}
 ...
 **/
static gboolean gpm_inhibit_applet_dbus_connect(GpmInhibitApplet *applet) {
  GError *error = NULL;
  ...
  if (applet->proxy == NULL) {
    ...
    applet->proxy = dbus_g_proxy_new_for_name_owner(
        applet->connection, GS_DBUS_SERVICE, GS_DBUS_PATH, GS_DBUS_INTERFACE,
        &error);
    if (error != NULL) {
 ...
 **/
static gboolean gpm_inhibit_applet_dbus_disconnect(GpmInhibitApplet *applet) {
  if (applet->proxy != NULL) {
 ...
 **/
static void gpm_inhibit_applet_name_appeared_cb(GDBusConnection *connection,
                                                const gchar *name,
 ...
 **/
static void gpm_inhibit_applet_name_vanished_cb(GDBusConnection *connection,
                                                const gchar *name,
                                                GpmInhibitApplet *applet) {
...
static void gpm_inhibit_applet_init(GpmInhibitApplet *applet) {
  ...
  applet->bus_watch_id = g_bus_watch_name(
      G_BUS_TYPE_SESSION, GS_DBUS_SERVICE, G_BUS_NAME_WATCHER_FLAGS_NONE,
      (GBusNameAppearedCallback)gpm_inhibit_applet_name_appeared_cb,
      (GBusNameVanishedCallback)gpm_inhibit_applet_name_vanished_cb, applet,
      NULL);
...
static gboolean gpm_applet_cb(MatePanelApplet *_applet, const gchar *iid,
  ...
  mate_panel_applet_setup_menu_from_file(
      MATE_PANEL_APPLET(applet), INHIBIT_MENU_UI_DIR "/inhibit-applet-menu.xml",
      action_group);
