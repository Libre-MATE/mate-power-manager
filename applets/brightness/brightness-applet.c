...
#define GPM_BRIGHTNESS_APPLET(o)                               \
  (G_TYPE_CHECK_INSTANCE_CAST((o), GPM_TYPE_BRIGHTNESS_APPLET, \
                              GpmBrightnessApplet))
#define GPM_BRIGHTNESS_APPLET_CLASS(k)                      \
  (G_TYPE_CHECK_CLASS_CAST((k), GPM_TYPE_BRIGHTNESS_APPLET, \
                           GpmBrightnessAppletClass))
#define GPM_IS_BRIGHTNESS_APPLET(o) \
  (G_TYPE_CHECK_INSTANCE_TYPE((o), GPM_TYPE_BRIGHTNESS_APPLET))
#define GPM_IS_BRIGHTNESS_APPLET_CLASS(k) \
  (G_TYPE_CHECK_CLASS_TYPE((k), GPM_TYPE_BRIGHTNESS_APPLET))
#define GPM_BRIGHTNESS_APPLET_GET_CLASS(o)                    \
  (G_TYPE_INSTANCE_GET_CLASS((o), GPM_TYPE_BRIGHTNESS_APPLET, \
                             GpmBrightnessAppletClass))
...
static void gpm_applet_stop_scroll_events_cb(GtkWidget *widget,
                                             GdkEvent *event);
...
static gboolean gpm_applet_key_press_cb(GtkWidget *popup, GdkEventKey *event,
                                        GpmBrightnessApplet *applet);
static gboolean gpm_applet_scroll_cb(GpmBrightnessApplet *applet,
...
static gboolean gpm_applet_cb(MatePanelApplet *_applet, const gchar *iid,
                              gpointer data);
...
#define MATE_PANEL_APPLET_VERTICAL(p)        \
  (((p) == MATE_PANEL_APPLET_ORIENT_LEFT) || \
   ((p) == MATE_PANEL_APPLET_ORIENT_RIGHT))
 ...
 **/
static gboolean gpm_applet_get_brightness(GpmBrightnessApplet *applet) {
  GError *error = NULL;
  gboolean ret;
  ...
  ret =
      dbus_g_proxy_call(applet->proxy, "GetBrightness", &error, G_TYPE_INVALID,
                        G_TYPE_UINT, &policy_brightness, G_TYPE_INVALID);
  if (error) {
    g_debug("ERROR: %s\n", error->message);
    g_error_free(error);
  }
 ...
 * Return value: Success value, or zero for failure
 **/
static gboolean gpm_applet_set_brightness(GpmBrightnessApplet *applet) {
  GError *error = NULL;
  gboolean ret;
  ...
  ret = dbus_g_proxy_call(applet->proxy, "SetBrightness", &error, G_TYPE_UINT,
                          applet->level, G_TYPE_INVALID, G_TYPE_INVALID);
  if (error) {
...
static void gpm_applet_get_icon(GpmBrightnessApplet *applet) {
  ...
  applet->icon = gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), icon,
                                          applet->size - 2, 0, NULL);
...
static void gpm_applet_check_size(GpmBrightnessApplet *applet) {
  ...
  if (MATE_PANEL_APPLET_VERTICAL(
    ...
    if (applet->size != allocation.width) {
      ...
      gtk_widget_set_size_request(GTK_WIDGET(applet), applet->size,
                                  applet->icon_height + 2);
    }
  } else {
    if (applet->size != allocation.height) {
      applet->size = allocation.height;
      gpm_applet_get_icon(applet);
      gtk_widget_set_size_request(GTK_WIDGET(applet), applet->icon_width + 2,
                                  applet->size);
    }
  }
}
...
static gboolean gpm_applet_draw_cb(GpmBrightnessApplet *applet) {
  ...
  bg_type = mate_panel_applet_get_background(MATE_PANEL_APPLET(applet), &color,
                                             &pattern);
  ...
  /* draw icon at center */
  gdk_cairo_set_source_pixbuf(cr, applet->icon, (w - applet->icon_width) / 2,
                              (h - applet->icon_height) / 2);
  cairo_paint(cr);
 ...
 **/
static gboolean gpm_applet_slide_delayed_cb(gpointer data) {
...
static gboolean gpm_applet_slide_cb(GtkWidget *w, GpmBrightnessApplet *applet) {
  ...
  if (!applet->slider_delay_id)
    applet->slider_delay_id =
        g_timeout_add(GPM_BRIGHTNESS_APPLET_SLIDER_FREQUENCY,
 ...
 **/
static gboolean on_popup_button_press(GtkWidget *widget, GdkEventButton *event,
                                      GpmBrightnessApplet *applet) {
...
static void gpm_applet_create_popup(GpmBrightnessApplet *applet) {
  ...
  gint orientation = mate_panel_applet_get_orient(
      MATE_PANEL_APPLET(MATE_PANEL_APPLET(applet)));
  ...
  gtk_window_set_type_hint(GTK_WINDOW(applet->popup),
 ...
 **/
static void gpm_applet_theme_change_cb(GtkIconTheme *icon_theme,
                                       gpointer data) {
 ...
 **/
static void gpm_applet_dialog_about_cb(GtkAction *action, gpointer data) {
  static const gchar *authors[] = {"Benjamin Canou <bookeldor@gmail.com>",
  ...
  gtk_show_about_dialog(
      NULL, "program-name", GPM_BRIGHTNESS_APPLET_NAME, "version", VERSION,
      ...
      "wrap-license", TRUE, "website", PACKAGE_URL, NULL);
 ...
 * open gpm help
 **/
static void gpm_applet_help_cb(GtkAction *action, gpointer data) {
...
static void gpm_applet_destroy_cb(GtkWidget *widget) {
  ...
  if (applet->icon != NULL) g_object_unref(applet->icon);
}
 ...
 **/
static void gpm_brightness_applet_class_init(GpmBrightnessAppletClass *class) {
  /* nothing to do here */
}

static void brightness_changed_cb(DBusGProxy *proxy, guint brightness,
                                  GpmBrightnessApplet *applet) {
 ...
 **/
static gboolean gpm_brightness_applet_dbus_connect(
    GpmBrightnessApplet *applet) {
  GError *error = NULL;
  ...
  if (applet->proxy == NULL) {
    g_debug("get proxy\n");
    g_clear_error(&error);
    applet->proxy = dbus_g_proxy_new_for_name_owner(
        applet->connection, GPM_DBUS_SERVICE, GPM_DBUS_PATH_BACKLIGHT,
        GPM_DBUS_INTERFACE_BACKLIGHT, &error);
    if (error != NULL) {
    ...
    dbus_g_proxy_add_signal(applet->proxy, "BrightnessChanged", G_TYPE_UINT,
                            G_TYPE_INVALID);
 ...
 **/
static gboolean gpm_brightness_applet_dbus_disconnect(
    GpmBrightnessApplet *applet) {
  if (applet->proxy != NULL) {
 ...
 **/
static void gpm_brightness_applet_name_appeared_cb(
    GDBusConnection *connection, const gchar *name, const gchar *name_owner,
    GpmBrightnessApplet *applet) {
 ...
 **/
static void gpm_brightness_applet_name_vanished_cb(
...
static void gpm_brightness_applet_init(GpmBrightnessApplet *applet) {
  ...
  /* monitor the daemon */
  applet->bus_watch_id = g_bus_watch_name(
      G_BUS_TYPE_SESSION, GPM_DBUS_SERVICE, G_BUS_NAME_WATCHER_FLAGS_NONE,
      (GBusNameAppearedCallback)gpm_brightness_applet_name_appeared_cb,
      (GBusNameVanishedCallback)gpm_brightness_applet_name_vanished_cb, applet,
      NULL);
  ...
  g_signal_connect(gtk_icon_theme_get_default(), "changed",
                   G_CALLBACK(gpm_applet_theme_change_cb), applet);
}
...
static gboolean gpm_applet_cb(MatePanelApplet *_applet, const gchar *iid,
  ...
  static const GtkActionEntry menu_actions[] = {
      ...
      {"Help", "help-browser", N_("_Help"), NULL, NULL,
  ...
  mate_panel_applet_setup_menu_from_file(
      ...
      BRIGHTNESS_MENU_UI_DIR "/brightness-applet-menu.xml", action_group);
 ...
 **/
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY(/* the factory iid */
                                      GPM_BRIGHTNESS_APPLET_FACTORY_ID,
                                      /* generates brighness applets instead of
                                         regular mate applets  */
